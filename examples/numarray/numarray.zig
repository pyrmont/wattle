...
fn defs(env: *wattle.Env) wattle.Error!void {
    ...
    wattle.nfuns(env, "numarray", &.{
        wattle.reg("new", &new, "(numarray/new size)", "Creates a new numarray"),
        wattle.reg("scale", &scale, "(numarray/scale numarray factor)", "Scales numarray by factor"),
        wattle.reg("sum", &sum, "(numarray/sum numarray)", "Sums numarray"),
        wattle.reg("length", &length, "(numarray/length numarray)", "Returns the length of numarray"),
    });
}
