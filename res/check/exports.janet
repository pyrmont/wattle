#!/usr/bin/env janet
# What the runtime publishes, classified.  Writes `res/check/exports.txt`.
#
...
#
#     ./res/check/exports.janet           regenerate res/check/exports.txt
#     ./res/check/exports.janet --check   fail if the tree disagrees with it
...
(defn- native-needs
  ...
  A `.so` built against the `wattle` module is linked with
  ...
  `_wattle_init` is handed, so nothing is left for `dlopen` to resolve.
...
(defn- runtime-decl-paths
  ...
  **This has to be the whole tree, and increment 2b found out the hard way.**
  ...
  `janet_io_write` -- which `res/check/seam.txt` does not count either, because it
...
(defn render [rows]
  ...
  (def out @"")
  (buffer/push out
    "# The exported surface, classified.  Generated by `./res/check/exports.janet`;\n"
    "# do not edit.\n"
    "#\n"
    ...
    "# named do. That population is read from `res/check/gated.txt` rather\n"
