#!/usr/bin/env janet
# Every definition in the runtime source that carries `callconv(.c)`,
# classified by what
# reaches it that way.
#
#     ./res/check/callconv.janet           regenerate res/check/callconv.txt
#     ./res/check/callconv.janet --check    fail if the tree disagrees with it
#
# ## Why this exists
#
...
(def list-path "res/check/callconv.txt")
...
(defn main [& argv]
  ...
  (def out @"")
  (buffer/push out "# Definitions in `src/` carrying `callconv(.c)`, by what reaches them.\n")
  (buffer/push out "# Generated by `./res/check/callconv.janet`; do not edit.\n#\n")
