...
#
#     ./res/check/comments.janet                extract the default set
#     ./res/check/comments.janet --out DIR      write under DIR instead
#     ./res/check/comments.janet src/module.zig extract these paths only
#
# ## Why this exists
#
...
(defn- default-paths
  ...
  the style guide under `res/`. Walking a source tree reaches its own
  READMEs; `res/` is reached for its READMEs and its style guide alone,
  ...
  (while (< i (length argv))
    ...
    (cond
      ...
      (tools/die "unrecognised argument: " a "\n"
                 "usage: ./res/check/comments.janet [--out DIR] [PATH ...]")
