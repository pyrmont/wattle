...
#
#     ./res/bench/upstream.sh [corpus] [layouts]
#     ./res/bench/upstream.sh res/bench/interpreter/bench.janet 12
#
# ## Why this exists
#
...
# `res/bench/interpreter/run.sh` when the question is "did this increment
...
CORPUS="${1:-res/bench/interpreter/bench.janet}"
LAYOUTS="${2:-12}"
...
./res/bench/layout.sh "$WORK/master/build/janet" "$CORPUS" "$LAYOUTS" > "$WORK/c.txt"
./res/bench/layout.sh "$WORK/out/bin/wattle" "$CORPUS" "$LAYOUTS" > "$WORK/zig.txt"
