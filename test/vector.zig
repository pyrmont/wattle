//! Behavioral contract for the growable vector: `scratch_vector.zig`, which is now
...
//!
//! The allocator. `scratch_vector.zig` chooses **scratch** over `utils.heap`, and
...
//! the tree asserts. If `scratch_vector.zig` were ever pointed at `utils.heap`, the
...
pub fn run() void {
    ...
    //                                               will hand out
    //
    // `janet_smalloc` puts its payload behind a `ScratchBlock` header, so the
