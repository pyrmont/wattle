#!/usr/bin/env janet
# Every `pub` declaration in `src/zig` that nothing in the tree references.
#
#     ./tools/check/orphans.janet           regenerate tools/check/orphans.txt
#     ./tools/check/orphans.janet --check    fail if the tree disagrees with it
#
# ## Why this exists
#
...
#
# The declared name, spelled anywhere in `src/zig`, `test/`, `examples/` or
...
(def list-path "tools/check/orphans.txt")
...
(def surface-files
  ...
  {"src/zig/abi.zig" true "src/zig/module.zig" true "src/zig/capi.zig" true})
...
(defn main [& argv]
  ...
  (def sources @{})
  (each path (array/concat @[] (tools/zig-files "src/zig")
  ...
  (def out @"")
  (buffer/push out "# `pub` declarations in `src/zig` that nothing in the tree references.\n")
  (buffer/push out "# Generated by `./tools/check/orphans.janet`; do not edit.\n#\n")
