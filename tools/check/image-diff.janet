...
#
#     ./tools/check/image-diff.janet                 # build the image; report size and host paths
#     ./tools/check/image-diff.janet --paths         # list the absolute host paths it embeds
#     ./tools/check/image-diff.janet --save FILE     # build it and keep a copy at FILE
#     ./tools/check/image-diff.janet --against FILE  # build it and compare with a saved copy
#
...
(def cache "/tmp/janet-image-cache")
...
(defn- build
  ...
  (def path (string out-prefix "/janet-image.bin"))
  (unless (os/stat path)
    (tools/die "image-diff.janet: no janet-image.bin in " out-prefix))
