...
# in `src/zig/value/tables.zig`, by way of a declaration nothing checks against
...
#
#     grep -rho 'c\.janet_[A-Za-z0-9_]*' src/zig --include='*.zig'
#
...
# `src/zig` does -- 5,018 against 3,430 -- because a Zig contract is compiled
# into a second copy of the runtime and reaches its subject the same way the
# runtime does. 85 names appear there and nowhere in `src/zig`. This is Phase
...
# ## Usage
#
#     ./tools/check/seam.janet            regenerate tools/check/seam.txt, print the summary
#     ./tools/check/seam.janet --check    compare the tree against tools/check/seam.txt
#     ./tools/check/seam.janet --quiet    the summary only
#
...
(def list-path "tools/check/seam.txt")
...
# `tools/README.md`'s warning arriving on schedule.
(def- export-at-peg
...
# Increment 5h batch A moved every `@export` into `src/zig/capi.zig`, and this
# tool started answering `src/zig/capi.zig` for all 86 names -- true, and
...
(def capi-path "src/zig/capi.zig")
    ...
    (cond
      ...
      (when inside
        (when-let [m (peg/match pat line)]
          (put out (m 0) (string "src/zig/" (m 1)))))))
  out)
...
(defn- render
  ...
  (def out @"")
  (buffer/push out
    "# The internal C-ABI seam.  Generated by `./tools/check/seam.janet`; do not edit.\n"
    "#\n"
    "# Every `c.janet_*` name spelled in Zig code under `src/zig` and `test/`,\n"
  ...
  (buffer/push out "#\n")
  (buffer/push out (string/format "# references     %5d  in src/zig\n" (counts :src-refs)))
  (buffer/push out (string/format "#                %5d  in test/\n" (counts :test-refs)))
  (buffer/push out
    "#\n"
    "# columns: refs  src/zig  test/  mechanism  name  definition\n"
    "\n")
  (each row rows
...
(defn main [& argv]
  ...
  (def paths (filter |(not= $ "src/zig/cabi_check.zig")
                     (array/concat (tools/zig-files "src/zig") (tools/zig-files "test"))))
  (def sources @{})
  ...
  (unless quiet
    ...
    (print (counts :refs) " references ("
           (counts :src-refs) " in src/zig, " (counts :test-refs) " in test/)"))
