...
#
#     ./tools/check/layouts.janet           regenerate tools/check/layouts.txt
#     ./tools/check/layouts.janet --check   fail if the tree disagrees with it

(import ../common :as tools)

(def list-path "tools/check/layouts.txt")
...
(defn render [rows]
  ...
  (buffer/push out
    ...
    "# Generated by `./tools/check/layouts.janet`; do not edit.\n"
    "#\n"
    "# `abi`   the type, or a pointer to it, is in a `callconv(.c)` signature\n"
    "# `field` it is a by-value field of another `extern` layout\n"
    "# `repr`  something reads its representation -- `@sizeOf`, `@offsetOf`,\n"
...
(defn verify
  ...
  then fails *downstream* -- at `janet-boot`, with an abort rather than a
  ...
  **It never writes the live tree.** It copies the repository into a scratch
  ...
  (cond
    ...
    (do (print "the compiler accepts every one, and so does the build.")
        (print "**That is a finding**: nothing left in `tools/check/layouts.txt` is")
