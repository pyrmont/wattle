...
#
#     ./tools/testing/contract.sh marsh             # test/marsh.zig
#     ./tools/testing/contract.sh marsh peg io_core # several, stopping at nothing
#
...
# plus the one contract plus `janet-contract-support.o` against
...
# one.** Since Phase 11 Part 27 `janet-zig-contract-test a b c` runs the three
...
# This script lives in `tools/`; every path below is relative to the
# repository root, one level up.
root=$(cd "$(dirname "$0")/../.." && pwd)
cd "$root"

if [ $# -eq 0 ]; then
    echo "usage: tools/testing/contract.sh <name> [name ...]   # test/<name>.zig" >&2
    exit 2
fi
