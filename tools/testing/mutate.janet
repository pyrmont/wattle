...
#
# Development instrument in `tools/`. The judge is the same for every source,
# so there is no per-increment edit.
#
#     ./tools/testing/mutate.janet --src src/runtime/ev.zig
#     ./tools/testing/mutate.janet --src src/runtime/ev.zig --only 3,17,64
#     ./tools/testing/mutate.janet --src A --only 3,17 --src B --only 5 --no-strings
#     unsetopt BG_NICE; nohup ./tools/testing/mutate.janet --src A --src B \
#       --no-strings --log /tmp/2a.log &
#     ./tools/testing/mutate.janet --src A --src B --log /tmp/2a.log --resume /tmp/2a.log
#     ./tools/testing/mutate.janet --all --log /tmp/janet-mutate.log
#     ./tools/testing/mutate.janet --src src/runtime/ev.zig --stage full
#
...
#
#     +- run exe janet-boot (janet-image.bin) failure        <- the generator
#     +- compile exe janet-boot Debug native-native 19 errors <- the compiler
#
...
(defn- judge-default []
  ...
  (label verdict
    ...
    (def smoke (twice "smoke"
                      (fn [] (tools/sh (string prefix "/bin/janet -e '(print (+ 1 2))'")
                                       :timeout bound))))
    ...
    (each suite (suite-list)
      (when-let [why (suite-failed (twice (string "suite:" (last (string/split "/" suite)))
                                          (fn [] (tools/sh (string prefix "/bin/janet " suite)
                                                           :timeout bound))))]
        (return verdict
...
(defn- warm-up
  ...
  (def bad (filter |(suite-failed (tools/sh (string prefix "/bin/janet " $) :timeout bound))
                   (suite-list)))
  (unless (empty? bad)
...
(defn- report
  ...
  (each src (sort (keys survivors))
    ...
    (unless (empty? ns)
      ...
      (printf "  ./tools/testing/mutate.janet --src %s%s --only %s"
  ...
  # chained.
  (when (> (length parts) 1)
    (printf "  one run: ./tools/testing/mutate.janet%s%s" strings-flag (string/join parts)))
  (:flush stdout))
