...
*Moved verbatim from the former root `AGENTS.md` on 2026-08-30. Read this before configuring or running `tools/testing/matrix.janet`, interpreting its timing and flaky results, or changing its contract population.*

## The acceptance matrix

`tools/testing/matrix.janet`. Set `contracts-default` at its head to the increment's own contract
files — that is the only per-increment edit it needs.

**`zig fmt` is `zig fmt src test build.zig`, and not `port/` or `tools/`.** The spike
...
arguments measures the harness, and the answer is the floor directly:
    ...
    ./tools/bench/value/run.sh ./janet ./janet 5     # should be zeros
...
candidate hit it once across twelve sizes.
    ...
    $ ./janet tools/bench/interpreter/bench.janet | grep pegmatch          # zsh
    pegmatch 0.038912
    $ bash -c './janet tools/bench/interpreter/bench.janet' | grep pegmatch
    pegmatch 0.057662
    $ PAD= bash -c './janet tools/bench/interpreter/bench.janet' | grep pegmatch
    pegmatch 0.039423
...
which reported it twice, at +48.4% and +47.8%, stable to three digits.
    ...
    ./tools/bench/layout.sh ./base/janet tools/bench/interpreter/bench.janet 12
    ./tools/bench/layout.sh ./cand/janet tools/bench/interpreter/bench.janet 12
...
`janet` binary — is what goes into the container.** `AGENTS.md`'s warning about
