...
READMEs under `src/`, `test/`, `tools/` and `examples/`, the example sources,
...
elements are allocated before `janet.pullAbstract` sits above the
allocation.
...
`janet.pullAbstract`: from the moment that returns, the block is on the
...
**In a file under `examples/`, a name reached through `@import("janet")` is
written `janet.name`.** The alias a reader writes is the author's alias. A
declaration in the same file stays bare, and a callback slot named as a
field (`gc`, `put`) stays bare. `numarray.zig` `new`: "`janet.new` returns
a block"; `defs`: "`janet.registerAbstract` refuses".
