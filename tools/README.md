...
Everything here is run by hand, with one exception: `zig build` compiles
`check/wasm_imports.zig` and runs it against each wasm32-wasi binary it
produces.
...
grep `tools/` for the name and update the scripts in the same change.
