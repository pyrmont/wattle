...
| `references.janet` | every identifier a comment in `src/` or `test/` names that the tree does not declare, resolved against every Zig file in the repository with comments stripped → `references.txt`. A comment describing the code beneath it does not drift; a comment naming *another* declaration does, because nothing binds the name in the prose to the name in the code, and a rename leaves every mention behind with no build, test or instrument saying a word. Measured at Phase 17 Part 2b: **239 of 262 camelCase names in comments resolved and 24 of 305 `janet_*` names did** — not a decay rate but one un-propagated rename, C to Zig. Two shapes are gated because both are this tree's own: `camelCase` (Zig's function convention) and `janet_*` (the C-era symbols). `SCREAMING_CASE`, `snake_case` and `Capitalized` are **not** gated, because a comment naming a C macro, a libc type or a std name is doing its job — gating them costs 393 rows of which almost all are legitimate, measured before the file was written rather than assumed. A `std.`-qualified span is skipped for the same reason. **`test/` is the larger half** — 328 rows to `src/`'s 262 — and `harness.zig` is the worst of it, teaching the protected-scope API to every contract author using two names the tree does not have, four lines above a `defer` that calls the real one. Classes are `c-era`, a bounded backlog that reaches zero when the prose pass does, and `unresolved`, where **the gate is that no row is new**; the header counts each per directory so that finishing `src/` stays visible while `test/` has not started. Five of its rows were found by reading in the session that wrote it, every one having survived a pass whose whole subject was the comments |
| `docblocks.janet` | every `///` block that a blank line or a `//` line separates from the declaration it documents → `docblocks.txt`. **A doc comment attaches to the next declaration however far away it is**, so an inserted section between a block and its subject silently moves the documentation onto whatever follows the gap, and nothing else in the tree can see it: `zig fmt` is indifferent, `build.zig` has no opinion, and `references.janet` asks whether the names in a comment resolve, which they still do — the sentence is true and attached to the wrong thing. Written at Phase 18 Part 8 because Part 7b had introduced one: `getRange`'s doc ended up on `bytesView` and survived a build, a full gate and a second session's review *of that block's prose*, exposed two parts later only by an unrelated name collision. A scan then found two more, one benign and one a real misattribution, and they are the same shape — which is the argument for gating on shape rather than on a reading. One class, `stranded`, which **must be empty**; it was empty on the first run because both known sites were fixed first. It reads shape and not meaning, so a block correctly adjacent to the *wrong* declaration stays invisible |
...
in a file something sits. `docblocks.txt` carries them too and for the same
reason: a stranded block is identified by where it is.
