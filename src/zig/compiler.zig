...
///
/// It was a `bool` beside two out-parameters, one of which -- the special --
/// had to survive the `false` that ended the loop, which is why it could not
/// simply be the return value. Three outcomes and one of them carrying a
/// payload is a union.
...
/// this file rather than a program error. Same shape as `gc/sweep.zig`'s
/// `assertFinalized`.
