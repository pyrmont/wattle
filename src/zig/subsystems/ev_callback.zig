...
//! kept a C face that jumped. What is left of that signature is a *layout*:
...
pub inline fn of(slot: c.JanetEVCallback) EVCallback {
    return @ptrCast(slot.?);
}

/// The same pointer on its way back into that storage, at registration.
pub inline fn stored(callback: anytype) c.JanetEVCallback {
...
pub inline fn dispatchTotal(
    ...
    event: c.JanetAsyncEvent,
) void {
    callback(fiber, event) catch c.janet_zig_fatal(
