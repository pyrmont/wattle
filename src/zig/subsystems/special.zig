...
//! head, `fn` on a malformed parameter list. Each kept a face that jumped.
//!
...
pub const Special = extern struct {
    name: [*c]const u8,
    compile: ?*const fn (c.JanetFopts, i32, [*c]const c.Janet) raise.Error!c.JanetSlot = null,
};

comptime {
    if (@sizeOf(Special) != @sizeOf(c.JanetSpecial)) {
...
pub inline fn of(slot: ?*const c.JanetSpecial) *const Special {
    return @ptrCast(@alignCast(slot.?));
}

/// The same pointer on its way back into that storage.
pub inline fn stored(s: *const Special) ?*const c.JanetSpecial {
    return @ptrCast(s);
}
