...
// The C-ABI faces of the four pushes. Written out rather than generated by
// `raise.panicking`, which builds a face for a function that returns a payload
// and has nothing to wrap where the return is `void`.
//
// These are what a C caller still gets, and they still jump. Part 17f deletes
// the `setjmp` they land in, and with it these four.

export fn janet_fiber_push(fiber: *c.JanetFiber, x: c.Janet) callconv(.c) void {
    raise.reported(push(fiber, x));
}

export fn janet_fiber_push2(fiber: *c.JanetFiber, x: c.Janet, y: c.Janet) callconv(.c) void {
    raise.reported(push2(fiber, x, y));
}

export fn janet_fiber_push3(fiber: *c.JanetFiber, x: c.Janet, y: c.Janet, z: c.Janet) callconv(.c) void {
    raise.reported(push3(fiber, x, y, z));
}

export fn janet_fiber_pushn(fiber: *c.JanetFiber, arr: [*c]const c.Janet, n: i32) callconv(.c) void {
    raise.reported(pushn(fiber, arr, n));
}
