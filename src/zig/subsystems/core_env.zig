...
/// The core image, defined by the generated `janet-image.c`. Referenced only
/// from the runtime branch, and a Zig extern declaration is lazy, so the
/// bootstrap build -- which has no image -- never asks the linker for it.
extern const janet_core_image: [*c]const u8;
extern const janet_core_image_size: usize;
