...
comptime {
    ...
    if (use_zig and has_dynamic_modules and windows) {
        ...
        @export(&symbolClibFace, .{ .name = "symbol_clib" });
    }
}

fn errorClibUnsupportedFace() callconv(.c) [*c]const u8 {
    return errorClibUnsupported();
}

fn loadClibFace(name: ?[*:0]const u8) callconv(.c) ?*anyopaque {
    return loadClib(name);
}

fn freeClibFace(lib: ?*anyopaque) callconv(.c) void {
    freeClib(lib);
}

fn errorClibFace() callconv(.c) [*c]const u8 {
    return errorClib();
}
