...
//! `JanetOverlapped` lives in `src/core/util.h`, which `abi.zig` deliberately
//! does not translate, so its Windows body is restated in Zig over
...
const backend = @import("ev_backend.zig");
...
const stream_unregistered: u32 = @intCast(c.JANET_STREAM_UNREGISTERED);
...
const stream_not_closeable: u32 = @intCast(c.JANET_STREAM_NOT_CLOSEABLE);
...
inline fn invalidHandle() c.JanetHandle {
...
pub export fn janet_ev_lasterr() callconv(.c) c.Janet {
    if (windows) {
        ...
        return c.janet_cstringv(&msgbuf);
    }
    return c.janet_cstringv(ev.janet_strerror(ev.errno()));
}
...
const default_methods = [_]corefn.Method{
...
fn janet_stream_extImpl(
    ...
    ev.assert(@src(), size >= @sizeOf(c.JanetStream), "bad size");
    const s: *c.JanetStream = @ptrCast(@alignCast(c.janet_abstract(abstract_type.stored(&janet_stream_type), size)));
    s.handle = handle;
    s.flags = flags;
    s.read_fiber = null;
    s.write_fiber = null;
    s.methods = if (methods == null) &default_methods else methods;
    s.index = 0;
    try backend.registerStream(s);
    return s;
}

export fn janet_stream_ext(
    handle: c.JanetHandle,
    flags: u32,
    methods: [*c]const c.JanetMethod,
    size: usize,
...
fn closeImplHandle(s: *c.JanetStream) raise.Raising(void) {
...
pub fn streamClose(s: *c.JanetStream) raise.Raising(void) {
    const rf = s.read_fiber;
    const wf = s.write_fiber;
    if (rf != null and rf.*.ev_callback != null) {
        try ev_callback.of(rf.*.ev_callback)(rf, c.JANET_ASYNC_EVENT_CLOSE);
        s.read_fiber = null;
    }
    if (wf != null and wf.*.ev_callback != null) {
        try ev_callback.of(wf.*.ev_callback)(wf, c.JANET_ASYNC_EVENT_CLOSE);
        s.write_fiber = null;
    }
    try closeImplHandle(s);
}

export fn janet_stream_close(s: *c.JanetStream) callconv(.c) void {
    raise.reported(streamClose(s));
}

/// Close a stream that was marked `TOCLOSE` once nothing is listening on it.
pub fn checkToClose(s: *c.JanetStream) raise.Raising(void) {
...
fn streamMark(p: ?*anyopaque, s: usize) callconv(.c) c_int {
    ...
    if (stream.read_fiber != null) c.janet_mark(c.janet_wrap_fiber(stream.read_fiber));
    if (stream.write_fiber != null) c.janet_mark(c.janet_wrap_fiber(stream.write_fiber));
    return 0;
}

fn streamGetter(p: ?*anyopaque, key: c.Janet, out: [*c]c.Janet) raise.Raising(c_int) {
    const stream: *c.JanetStream = @ptrCast(@alignCast(p));
    if (c.janet_checktype(key, c.JANET_KEYWORD) == 0) return 0;
    return c.janet_getmethod(c.janet_unwrap_keyword(key), @ptrCast(@alignCast(stream.methods)), out);
}

fn streamMarshal(p: ?*anyopaque, ctx: [*c]c.JanetMarshalContext) raise.Raising(void) {
    const s: *c.JanetStream = @ptrCast(@alignCast(p));
    ...
    try marshalling.marshalInt(ctx, @bitCast(s.flags));
    ...
    if (windows) {
        ...
        try marshalling.marshalInt64(ctx, @bitCast(@intFromPtr(duph)));
    } else {
        // Marshal after dup because it is easier than maintaining our own
        // reference counting.
        const duph = ev.dup(s.handle);
        if (duph < 0) return pp_format.panicf("failed to duplicate stream handle: %V", .{janet_ev_lasterr()});
        try marshalling.marshalInt(ctx, duph);
    }
}

fn streamUnmarshal(ctx: [*c]c.JanetMarshalContext) raise.Raising(?*anyopaque) {
    if (c.janet_unmarshal_flags(ctx) & c.JANET_MARSHAL_UNSAFE == 0) {
        return raise.panic("can only unmarshal stream with unsafe flag");
    }
    const p: *c.JanetStream = @ptrCast(@alignCast(try marsh.unmarshalAbstract(ctx, @sizeOf(c.JanetStream))));
...
fn streamNext(p: ?*anyopaque, key: c.Janet) raise.Raising(c.Janet) {
    const stream: *c.JanetStream = @ptrCast(@alignCast(p));
    return c.janet_nextmethod(@ptrCast(@alignCast(stream.methods)), key);
}
...
fn streamToString(p: ?*anyopaque, buffer: [*c]c.JanetBuffer) raise.Raising(void) {
    const stream: *c.JanetStream = @ptrCast(@alignCast(p));
...
pub fn streamFlags(s: *c.JanetStream, flags: u32) raise.Raising(void) {
...
export fn janet_stream_flags(s: *c.JanetStream, flags: u32) callconv(.c) void {
...
fn ev_callback_read(fiber: [*c]c.JanetFiber, event: c.JanetAsyncEvent) raise.Raising(void) {
    const s: *c.JanetStream = fiber.*.ev_stream;
    const state: *StateRead = @ptrCast(@alignCast(fiber.*.ev_state));
    switch (event) {
        c.JANET_ASYNC_EVENT_MARK => c.janet_mark(c.janet_wrap_buffer(state.buf)),
        c.JANET_ASYNC_EVENT_CLOSE => {
            ev.janet_schedule(fiber, c.janet_wrap_nil());
            ev.janet_async_end(fiber);
        },
        else => {
            if (windows) {
...
fn readWindows(fiber: [*c]c.JanetFiber, s: *c.JanetStream, state: *StateRead, event: c.JanetAsyncEvent) raise.Raising(void) {
    var start_transfer = false;
    switch (event) {
        c.JANET_ASYNC_EVENT_FAILED, c.JANET_ASYNC_EVENT_COMPLETE => {
            ...
            _ = try containers.bufferPushBytes(state.buf, &state.chunk_buf, @intCast(ev_bytes));
            state.bytes_left -= @intCast(ev_bytes);
            if (state.bytes_left == 0 or state.is_chunk == 0 or ev_bytes == 0) {
                var resume_val: c.Janet = undefined;
                if (has_net and state.mode == read_mode_recvfrom) {
                    const abst = c.janet_abstract(abstract_type.stored(&ev.janet_address_type), @intCast(state.fromlen));
                    @memcpy(@as([*]u8, @ptrCast(abst))[0..@intCast(state.fromlen)], state.from[0..@intCast(state.fromlen)]);
                    resume_val = c.janet_wrap_abstract(abst);
                } else {
                    resume_val = c.janet_wrap_buffer(state.buf);
                }
                ev.janet_schedule(fiber, resume_val);
                ev.janet_async_end(fiber);
                return;
            }
            start_transfer = true;
        },
        c.JANET_ASYNC_EVENT_INIT => start_transfer = true,
        else => {},
    }
    ...
    if (has_net and state.mode == read_mode_recvfrom) {
        ...
        if (status != 0 and WSAGetLastError() != WSA_IO_PENDING) {
            try ev.cancel(fiber, janet_ev_lasterr());
            ev.janet_async_end(fiber);
            return;
        }
    } else {
        ...
        if (status == 0 and GetLastError() != ERROR_IO_PENDING) {
            if (GetLastError() == ERROR_BROKEN_PIPE) {
                if (state.bytes_read != 0) {
                    ev.janet_schedule(fiber, c.janet_wrap_buffer(state.buf));
                } else {
                    ev.janet_schedule(fiber, c.janet_wrap_nil());
                }
            } else {
...
fn readPosix(fiber: [*c]c.JanetFiber, s: *c.JanetStream, state: *StateRead, event: c.JanetAsyncEvent) raise.Raising(void) {
    switch (event) {
        c.JANET_ASYNC_EVENT_ERR => {
            if (state.bytes_read != 0) {
                ev.janet_schedule(fiber, c.janet_wrap_buffer(state.buf));
            } else {
                ev.janet_schedule(fiber, c.janet_wrap_nil());
        ...
        c.JANET_ASYNC_EVENT_HUP, c.JANET_ASYNC_EVENT_INIT, c.JANET_ASYNC_EVENT_READ => {
            ...
            while (true) {
                ...
                const read_limit: i32 = if (state.is_chunk != 0)
                    ...
                    bytes_left;
                try containers.bufferExtra(buffer, read_limit);
                ...
                if (state.is_chunk == 0 or bytes_left == 0 or nread == 0) {
                    var resume_val: c.Janet = undefined;
                    if (has_net and state.mode == read_mode_recvfrom) {
                        const abst = c.janet_abstract(abstract_type.stored(&ev.janet_address_type), socklen);
                        @memcpy(@as([*]u8, @ptrCast(abst))[0..socklen], saddr[0..socklen]);
                        resume_val = c.janet_wrap_abstract(abst);
                    } else {
                        resume_val = c.janet_wrap_buffer(buffer);
                    }
                    ev.janet_schedule(fiber, resume_val);
                    ev.janet_async_end(fiber);
                    return;
...
pub fn readGeneric(
    ...
    const state: *StateRead = @ptrCast(@alignCast(c.janet_malloc(@sizeOf(StateRead)) orelse
        ev.outOfMemory(@src())));
    ...
    return ev.asyncStart(s, c.JANET_ASYNC_LISTEN_READ, ev_callback_read, state);
}

export fn janet_ev_read(s: *c.JanetStream, buf: [*c]c.JanetBuffer, nbytes: i32) callconv(.c) void {
    raise.report(readGeneric(s, buf, nbytes, false, read_mode_read, 0));
}

export fn janet_ev_readchunk(s: *c.JanetStream, buf: [*c]c.JanetBuffer, nbytes: i32) callconv(.c) void {
    raise.report(readGeneric(s, buf, nbytes, true, read_mode_read, 0));
}

comptime {
    if (has_net) {
        @export(&evRecv, .{ .name = "janet_ev_recv" });
        @export(&evRecvChunk, .{ .name = "janet_ev_recvchunk" });
        @export(&evRecvFrom, .{ .name = "janet_ev_recvfrom" });
        @export(&evSendBuffer, .{ .name = "janet_ev_send_buffer" });
        @export(&evSendString, .{ .name = "janet_ev_send_string" });
        @export(&evSendToBuffer, .{ .name = "janet_ev_sendto_buffer" });
        @export(&evSendToString, .{ .name = "janet_ev_sendto_string" });
    }
}

fn evRecv(s: *c.JanetStream, buf: [*c]c.JanetBuffer, nbytes: i32, flags: c_int) callconv(.c) void {
    raise.report(readGeneric(s, buf, nbytes, false, read_mode_recv, flags));
}

fn evRecvChunk(s: *c.JanetStream, buf: [*c]c.JanetBuffer, nbytes: i32, flags: c_int) callconv(.c) void {
    raise.report(readGeneric(s, buf, nbytes, true, read_mode_recv, flags));
}

fn evRecvFrom(s: *c.JanetStream, buf: [*c]c.JanetBuffer, nbytes: i32, flags: c_int) callconv(.c) void {
...
fn ev_callback_write(fiber: [*c]c.JanetFiber, event: c.JanetAsyncEvent) raise.Raising(void) {
    const s: *c.JanetStream = fiber.*.ev_stream;
    const state: *StateWrite = @ptrCast(@alignCast(fiber.*.ev_state));
    switch (event) {
        c.JANET_ASYNC_EVENT_MARK => {
            c.janet_mark(if (state.is_buffer != 0)
                c.janet_wrap_buffer(state.src.buf)
            else
                c.janet_wrap_string(state.src.str));
            if (state.mode == write_mode_sendto) {
                c.janet_mark(c.janet_wrap_abstract(state.dest_abst));
            }
        },
        c.JANET_ASYNC_EVENT_CLOSE => {
            try ev.cancel(fiber, c.janet_cstringv("stream closed"));
            ev.janet_async_end(fiber);
        },
        else => {
            if (windows) {
...
fn writeWindows(fiber: [*c]c.JanetFiber, s: *c.JanetStream, state: *StateWrite, event: c.JanetAsyncEvent) raise.Raising(void) {
    switch (event) {
        c.JANET_ASYNC_EVENT_FAILED, c.JANET_ASYNC_EVENT_COMPLETE => {
            const ev_bytes: u32 = @truncate(state.overlapped.bytes_transfered);
            if (ev_bytes == 0 and state.mode != write_mode_sendto) {
                try ev.cancel(fiber, c.janet_cstringv("disconnect"));
                ev.janet_async_end(fiber);
                return;
            }
            ev.janet_schedule(fiber, c.janet_wrap_nil());
        ...
        c.JANET_ASYNC_EVENT_INIT => {
            ...
            if (state.is_buffer != 0) {
                ...
                const str = c.janet_string(buffer.*.data, buffer.*.count);
                bytes = str;
                len = buffer.*.count;
                state.is_buffer = 0;
                state.src.str = str;
                ...
                bytes = state.src.str;
                len = c.janet_string_length(bytes);
            }
            state.overlapped = std.mem.zeroes(Overlapped);
            ...
            if (has_net and state.mode == write_mode_sendto) {
                ...
                const tolen: c_int = @intCast(c.janet_abstract_size(to));
...
fn writePosix(fiber: [*c]c.JanetFiber, s: *c.JanetStream, state: *StateWrite, event: c.JanetAsyncEvent) raise.Raising(void) {
    switch (event) {
        c.JANET_ASYNC_EVENT_ERR => {
            try ev.cancel(fiber, c.janet_cstringv("stream err"));
            ev.janet_async_end(fiber);
        },
        c.JANET_ASYNC_EVENT_HUP => {
            try ev.cancel(fiber, c.janet_cstringv("stream hup"));
            ev.janet_async_end(fiber);
        },
        c.JANET_ASYNC_EVENT_INIT, c.JANET_ASYNC_EVENT_WRITE => {
            var len: i32 = undefined;
            var bytes: [*c]const u8 = undefined;
            ...
            if (start < len) {
                ...
                while (true) {
                    ...
                    if (has_net and state.mode == write_mode_sendto) {
                        nwrote = sendto(s.handle, from, @intCast(nbytes), state.flags, dest_abst, @intCast(c.janet_abstract_size(dest_abst)));
...
pub fn writeGeneric(
    ...
    const state: *StateWrite = @ptrCast(@alignCast(c.janet_malloc(@sizeOf(StateWrite)) orelse
        ev.outOfMemory(@src())));
    ...
    return ev.asyncStart(s, c.JANET_ASYNC_LISTEN_WRITE, ev_callback_write, state);
}

export fn janet_ev_write_buffer(s: *c.JanetStream, buf: [*c]c.JanetBuffer) callconv(.c) void {
    raise.report(writeGeneric(s, buf, null, write_mode_write, true, 0));
}

export fn janet_ev_write_string(s: *c.JanetStream, str: [*c]const u8) callconv(.c) void {
    raise.report(writeGeneric(s, @constCast(str), null, write_mode_write, false, 0));
}

fn evSendBuffer(s: *c.JanetStream, buf: [*c]c.JanetBuffer, flags: c_int) callconv(.c) void {
    raise.report(writeGeneric(s, buf, null, write_mode_send, true, flags));
}

fn evSendString(s: *c.JanetStream, str: [*c]const u8, flags: c_int) callconv(.c) void {
    raise.report(writeGeneric(s, @constCast(str), null, write_mode_send, false, flags));
}

fn evSendToBuffer(s: *c.JanetStream, buf: [*c]c.JanetBuffer, dest: ?*anyopaque, flags: c_int) callconv(.c) void {
    raise.report(writeGeneric(s, buf, dest, write_mode_sendto, true, flags));
}

fn evSendToString(s: *c.JanetStream, str: [*c]const u8, dest: ?*anyopaque, flags: c_int) callconv(.c) void {
...
pub fn cfunStreamClose(argc: i32, argv: [*c]c.Janet) align(corefn.alignment) raise.Raising(c.Janet) {
    try arglayer.fixarity(argc, 1);
    try streamClose(try getStream(argv, 0));
    return argv[0];
}

pub fn cfunStreamRead(argc: i32, argv: [*c]c.Janet) align(corefn.alignment) raise.Raising(c.Janet) {
    ...
    if (c.janet_keyeq(argv[1], "all") != 0) {
        if (to != std.math.inf(f64)) ev.janet_addtimeout(to);
...
pub fn cfunStreamChunk(argc: i32, argv: [*c]c.Janet) align(corefn.alignment) raise.Raising(c.Janet) {
    try arglayer.arity(argc, 2, 4);
    const s = try getStream(argv, 0);
    try streamFlags(s, stream_readable);
    const n = try arglayer.getNat(argv, 1);
...
pub fn cfunStreamWrite(argc: i32, argv: [*c]c.Janet) align(corefn.alignment) raise.Raising(c.Janet) {
    ...
    if (c.janet_checktype(argv[1], c.JANET_BUFFER) != 0) {
        if (to != std.math.inf(f64)) ev.janet_addtimeout(to);
        return writeGeneric(s, try arglayer.getBuffer(argv, 1), null, write_mode_write, true, 0);
    }
...
fn getFileForStream(s: *c.JanetStream) raise.Raising(?*c.JanetFile) {
    ...
    return c.janet_makejfile(@ptrCast(@alignCast(f)), flags);
}

fn toFileImpl(argc: i32, argv: [*c]c.Janet) raise.Raising(c.Janet) {
    try arglayer.fixarity(argc, 1);
    const s = try getStream(argv, 0);
    const iof = try getFileForStream(s);
    if (iof == null) return raise.panic("cannot make file from stream");
    return c.janet_wrap_abstract(iof);
}
