...
//! same division Part 17g drew for `corefn.Method`: the C ABI is a layout, and
...
pub const AbstractType = extern struct {
    ...
    get: ?*const fn (?*anyopaque, c.Janet, [*c]c.Janet) raise.Error!c_int = null,
    put: ?*const fn (?*anyopaque, c.Janet, c.Janet) raise.Error!void = null,
    marshal: ?*const fn (?*anyopaque, [*c]c.JanetMarshalContext) raise.Error!void = null,
    unmarshal: ?*const fn ([*c]c.JanetMarshalContext) raise.Error!?*anyopaque = null,
    tostring: ?*const fn (?*anyopaque, [*c]c.JanetBuffer) raise.Error!void = null,
    compare: ?*const fn (?*anyopaque, ?*anyopaque) callconv(.c) c_int = null,
    hash: ?*const fn (?*anyopaque, usize) callconv(.c) i32 = null,
    next: ?*const fn (?*anyopaque, c.Janet) raise.Error!c.Janet = null,
    call: ?*const fn (?*anyopaque, i32, [*c]c.Janet) raise.Error!c.Janet = null,
    length: ?*const fn (?*anyopaque, usize) raise.Error!usize = null,
    bytes: ?*const fn (?*anyopaque, usize) callconv(.c) c.JanetByteView = null,
...
pub inline fn of(slot: ?*const c.JanetAbstractType) *const AbstractType {
...
pub inline fn ofAbstract(abst: ?*anyopaque) *const AbstractType {
    return of(c.janet_abstract_type(abst));
}

/// The same pointer on its way back into that storage.
pub inline fn stored(at: *const AbstractType) [*c]const c.JanetAbstractType {
    return @ptrCast(at);
}
