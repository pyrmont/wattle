...
/// One node of a serialized `JanetFFIType`, in pre-order: a struct's fields
...
///
/// `size` is `type_size` — the array count already multiplied in — while
...
fn sysv64ClassifyExt(nodes: []const TypeNode, idx: usize, shift: u64) Walk {
    ...
    return switch (node.prim) {
        ...
        // C panics "nyi" here, but every ordinal the enumeration defines is
        // handled above, so this arm stands only because the switch is over an
        // integer rather than an enumeration.
...
fn aapcs64Classify(nodes: []const TypeNode, idx: usize) Walk {
    ...
    return switch (node.prim) {
        ...
        // As in the SysV classifier, C panics "nyi" on an ordinal that the
        // enumeration cannot produce.
