...
//!
//! `JanetChannel` is opaque in Janet's header, so this file owns the layout.
//! Its last member is a `pthread_mutex_t` or a `CRITICAL_SECTION` --
//! `types.zig`'s in both arms, which is where every shared type in this tree
//! comes from, so this file needs no `@cImport` of its own.
//!
...
fn chanDeinit(chan: *Channel) void {
    if (isThreaded(chan)) {
        ...
        while (ev_core.qPop(&chan.items, &item, @sizeOf(repr.Value)) == 0) {
...
fn chanatGC(chan: *Channel, _: usize) c_int {
    chanDeinit(chan);
    return 0;
}
...
fn chanatGCPerThread(chan: *Channel, _: usize) c_int {
    lock(chan);
...
fn chanatMark(chan: *Channel, _: usize) c_int {
    markFQ(&chan.read_pending);
    markFQ(&chan.write_pending);
    const items = &chan.items;
    const data: [*]repr.Value = @ptrCast(@alignCast(items.data orelse return 0));
...
fn chanatGet(_: *Channel, key: repr.Value, out: *repr.Value) raise.Raising(c_int) {
...
fn chanatMarshal(chan: *Channel, ctx: *types.JanetMarshalContext) raise.Raising(void) {
...
fn chanatUnmarshal(ctx: *types.JanetMarshalContext) raise.Raising(*Channel) {
    ...
    while (i < count) : (i += 1) {
        ...
        ev.assert(@src(), ev_core.qPush(&abst.items, &item, @sizeOf(repr.Value)) == 0, "bad unmarshal channel");
    }
    return abst;
}
...
pub fn makeSupervisorEvent(name: [*:0]const u8, fiber: *types.JanetFiber, threaded: bool) repr.Value {
...
fn pushWithLock(chan: *Channel, x_in: repr.Value, mode: c_int) raise.Raising(bool) {
    var x = x_in;
    var reader: Pending = undefined;
    if (chan.closed != 0) {
        unlock(chan);
    ...
    if (is_empty != 0) {
        // No pending reader.
        if (ev_core.qPush(&chan.items, &x, @sizeOf(repr.Value)) != 0) {
...
pub fn push(chan: *Channel, x: repr.Value, mode: c_int) raise.Raising(bool) {
    lock(chan);
...
fn popWithLock(chan: *Channel, item: *repr.Value, is_choice: c_int) raise.Raising(bool) {
    var writer: Pending = undefined;
    if (chan.closed != 0) {
        unlock(chan);
        item.* = wrap.fromNil();
    ...
    const is_threaded = isThreaded(chan);
    if (ev_core.qPop(&chan.items, item, @sizeOf(repr.Value)) != 0) {
        // Queue empty.
...
pub fn pop(chan: *Channel, item: *repr.Value, is_choice: c_int) raise.Raising(bool) {
    lock(chan);
...
pub fn getChannel(argv: []const repr.Value, n: i32) raise.Raising(?*types.JanetChannel) {
    return @ptrCast(try args_core.getAbstract(argv, n, &channelType));
}
...
) raise.Raising(?*types.JanetChannel) {
    if (argv.len > n and !repr.checkType(argv[@intCast(n)], repr.Tag.nil)) {
        return getChannel(argv, n);
    }
    return dflt;
}

pub fn channelGive(chan: ?*types.JanetChannel, x: repr.Value) raise.Raising(bool) {
    return push(unwrap(chan), x, 2);
}

pub fn channelGiveAbi(chan: ?*types.JanetChannel, x: repr.Value) c_int {
    return @intFromBool(raise.reported(channelGive(chan, x)));
}

pub fn channelTake(chan: ?*types.JanetChannel, out: *repr.Value) raise.Raising(bool) {
    return pop(unwrap(chan), out, 2);
}

pub fn channelTakeAbi(chan: ?*types.JanetChannel, out: *repr.Value) c_int {
...
pub fn channelMake(limit: u32) ?*types.JanetChannel {
    ...
    const chan = unwrap(abstracts.new(&channelType, @sizeOf(Channel)));
...
fn channelArg(argv: []const repr.Value, n: i32) raise.Raising(*Channel) {
    return unwrap(try args_core.getAbstract(argv, n, &channelType));
}
...
fn cfunTake(argv: []repr.Value) raise.Raising(repr.Value) {
    ...
    if (try pop(chan, &item, 0)) ev.schedule(vm_state.current().root_fiber.?, item);
...
fn cfunChoice(argv: []repr.Value) raise.Raising(repr.Value) {
    ...
    var data: ?[*]const repr.Value = undefined;
...
fn cfunFull(argv: []repr.Value) raise.Raising(repr.Value) {
    ...
    const chan = try channelArg(argv, 0);
    lock(chan);
    const ret = wrap.fromBoolean(ev_core.qCount(&chan.items) >= chan.limit);
    unlock(chan);
    return ret;
}
