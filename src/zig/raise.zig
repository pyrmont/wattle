...
//! imports. It is shared like `abi.zig` rather than selected like a subsystem,
...
//! symbol keeps **two faces**, so the C face and the Zig face are each other's
...
pub fn signal(sig: c.JanetSignal, message: c.Janet) Error {
...
pub fn panicv(message: c.Janet) Error {
...
pub inline fn pendingSignal() c.JanetSignal {
...
pub inline fn pendingPayload() c.Janet {
...
pub inline fn stored(cfun: anytype) c.JanetCFunction {
    return @ptrCast(cfun);
}
...
///
/// So a face records the raise and returns a zeroed value, and its caller
...
/// runtime does not call it, and it dies with the faces in Part 18.
///
...
/// An all-zero value of any type a face can return.
///
/// `std.mem.zeroes` refuses a non-nullable pointer, and it is right to: zero is
/// not a value of `*JanetTable`. Three faces return one. Writing the bytes
...
///
/// This is what a hand-written C face is made of, and it takes the whole
...
/// the same thing for the faces that are derived rather than written.
...
/// face was made of until the hinge. `janet_panicv` and `janet_await` are the
...
/// than by import gets that neighbour's C face, which reports instead of
...
/// Whether a raise reached a C-ABI face since this was last asked. Clears.
///
...
/// is still the one place a converted subsystem's C face is built, and it is
/// still built rather than written out, because 177 hand-written faces that
...
/// lines each is not much until it is three hundred of them, and a face that
...
/// `noalias` is dropped, and a variadic function has no face. Nothing this
/// wraps is either.
pub fn panicking(comptime f: anytype) type {
    ...
    return switch (p.len) {
        0 => struct {
            pub fn face() callconv(.c) P {
                return f() catch reportToC(P);
            }
        },
        1 => struct {
            pub fn face(a: p[0].type.?) callconv(.c) P {
                return f(a) catch reportToC(P);
            }
        },
        2 => struct {
            pub fn face(a: p[0].type.?, b: p[1].type.?) callconv(.c) P {
                return f(a, b) catch reportToC(P);
            }
        },
        3 => struct {
            pub fn face(a: p[0].type.?, b: p[1].type.?, d: p[2].type.?) callconv(.c) P {
                return f(a, b, d) catch reportToC(P);
            }
        },
        4 => struct {
            pub fn face(a: p[0].type.?, b: p[1].type.?, d: p[2].type.?, e: p[3].type.?) callconv(.c) P {
                return f(a, b, d, e) catch reportToC(P);
            }
        },
        5 => struct {
            pub fn face(a: p[0].type.?, b: p[1].type.?, d: p[2].type.?, e: p[3].type.?, g: p[4].type.?) callconv(.c) P {
