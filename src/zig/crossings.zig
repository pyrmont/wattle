...
//! runtime declarations to obtain six**, and with them `FILE`, `JanetHandle`
...
// **They were unchecked, and two of them had drifted**: `janet_getmethod` and
// `janet_nextmethod` declared `abi.Method` where the definition takes
// `method_type.CMethod` -- two layouts that agreed only by luck -- and
// `janet_wrap_abstract` declared a non-optional pointer against a definition
// that accepts null. Neither is something a linker can catch, and an author's
// `.so` is the one compilation where getting it wrong is not this project's
// crash to debug.
