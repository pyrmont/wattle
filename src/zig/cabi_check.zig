...
/// flavour, because a translated `T *` is `[*c]T` and 139 of the pairs differed
/// on that and nothing else. No declaration in `cabi.zig` is translated now, so
/// the exemption protected nothing and hid something.
///
...
pub fn verify() void {
    comptime {
        ...
        // **The twenty-one `module.zig` calls, which were compared against
        // nothing.** They are the whole of the author boundary, and an author's
        // `.so` is the one compilation where a disagreement is not this
        // project's crash to debug. Two had drifted before this ran.
        report = report ++ checkFile(.{
