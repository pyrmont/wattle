...
//! through `api/interface.zig`'s struct of function pointers, and the runtime
//! reaches itself by import, so there is no Janet declaration here whose
//! definition lives in this tree and nothing left for a check to compare.
//! libc's own declarations are checked by the header the build links against.
//!
//! Zig 0.15.1 removed `usingnamespace`, and a flattening alias is what that
//! keyword was removed to discourage, so there are no re-exported aliases here
//! for a call site to reach a neighbour's declaration through.
//!
//! The VM's storage is not declared here and could not be. It is the one
//! object whose storage class follows the build, `threadlocal` unless
...
/// `struct itimerspec`, the other Linux shape the epoll backend passes by
/// pointer.
...
/// takes. `runtime/ev/stream.zig` embeds an `OVERLAPPED` in `Stream`'s
/// Windows arm.
...
/// `WSABUF`, the scatter/gather element the two overlapped socket calls
/// take.
...
/// Janet redirects `fseek` and `ftell` to the 64-bit Microsoft variants, so
/// the port calls what the C implementation calls rather than the narrow
/// ones.
...
/// `errno`, which is a macro in every libc and so not a symbol.
///
/// This is the accessor a caller reaches it through, and it belongs with the
/// rest of what is genuinely external.
...
/// `fmod` from `<math.h>`. `@rem` has the same rounding for finite operands
/// but is not defined over infinities the way the C library function is, and
...
/// Frees what a host call allocated.
///
/// `canonicalPath` is `realpath` on POSIX and `_fullpath` on Windows, both of
/// which allocate with the host's allocator, so the host's `free` is what
/// releases the result. A caller releases it with a `defer`, because the
/// interning between the allocation and the release can raise, and a
/// hand-written release below the raise is a release the raise goes straight
/// past.
pub extern fn free(ptr: ?*anyopaque) callconv(.c) void;

pub extern fn frexp(f64, *c_int) f64;

/// The POSIX seek pair, in its 64-bit spelling. `fseek` and `ftell` take a
/// `long`, which is 32 bits on a 32-bit POSIX host, where `off_t` is 64 bits
/// on every target this builds for and `file/seek`'s docstring promises files
/// of more than 4GB. On a 64-bit host the two pairs are the same call.
...
/// Calls `f(args)` again for as long as it fails because a signal interrupted
/// it, and returns what it finally returned.
///
/// `f` is the call and `args` its arguments. One loop rather than one per
/// caller, so the condition is written once and cannot be inverted in a copy.
///
/// The failure test is `< 0`, which is every call this wraps: `close`, `read`,
/// `write`, `open`, `kevent`, `epoll_wait`, `inotify_rm_watch`, `waitpid` and
/// the rest all return a negative on failure and set `errno`. A call whose
/// failure is not negative does not belong here, because it would need its own
/// predicate.
...
/// Writes `errno`.
///
/// `value` is the number to store. `runtime/os/fs.zig`'s directory walk is the
/// one caller: it clears `errno` before each `readdir`, because a null result
/// there means either the end of the stream or a failure and only `errno`
/// separates them. It is a setter rather than a `_errno()` at the call site so
/// that `std.c._errno` has exactly one mention in the tree, which is what makes
/// the seam checkable.
...
/// The XSI signature, which every libc in this project's reach except glibc
/// has.
