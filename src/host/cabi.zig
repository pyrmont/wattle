...
/// `EINTR`, once. Spelled in more than one place it is a chance for two of
/// them to mean different numbers on a target nobody built.
...
/// takes. `ev/stream.zig` embeds one in `Stream`'s Windows arm.
...
// from src/runtime/ev/stream.zig -- Winsock, which is its own import library
