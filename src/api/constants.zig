...
//! declaration the compiler can re-derive -- a wrong opcode number is a
//! working program giving a wrong answer rather than a build failure -- so
//! these were extracted from a translation of Janet's headers over 31
//! configurations and diffed rather than transcribed by hand.
//! `helpers.promoteIntLiteral` is kept for that reason: it is C's
//! integer-literal promotion, and the type it yields depends on the target's
//! `long`.
//!
...
//! except the ten above that follow a `-D` flag.
...
//
// The cost of letting the three drift is worth stating: written as the
// inverse predicate, with `registry.zig`'s `checkPointerAlign` masking against
// a separately spelled shift, the alignment check is off on aarch64 Linux --
// the only targets that shift at all -- and runs with a zero mask on aarch64
// macOS, checking nothing.
...
// The sixteen type values upstream enumerates are `repr.Tag`'s members, an
// `enum(u4)` in the module that owns the representation. The masks below are
// the one thing left that needs their numbering.
...
/// "both" is the two set rather than a third value.
...
/// funcdef carries it, and `disasm` prints its name. The block at the bottom
/// asserts every member against its number, so a renumbering is a compile
/// error rather than a silently different image.
///
...
pub const Opcode = enum(u8) {
    ...
    /// opcode. `verify.zig`'s type table
...
// an `int` convert in `capi.zig`, and the `.typecheck` instruction's operand
...
// positions are asserted there. `vm/lifecycle.zig`'s `sandbox` and
// `sandboxAssert` take that struct rather than a bit word.
