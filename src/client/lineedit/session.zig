...
pub const Session = struct {
    ...
    /// on this line, as `editor.Editor.finished` does. The result is how the
    ...
    pub fn begin(session: *Session, prompt: []const u8, size: Size, finished: ?editor_mod.Finished) error{OutOfMemory}!?End {
    ...
    pub fn feed(session: *Session, bytes: []const u8, size: Size) error{OutOfMemory}!?End {
        ...
        for (bytes, 0..) |byte, i| {
            ...
            const outcome = switch (key) {
                .up => session.editor.vertical(session.geometry(), true),
                .down => session.editor.vertical(session.geometry(), false),
...
test "begin: draws the prompt" {
    ...
    try std.testing.expectEqual(null, try session.begin("repl:1:> ", w80, null));
...
test "feed: one frame for a whole read, and the last frame at submission" {
    var session = Session.init(std.testing.allocator);
    defer session.deinit();
    _ = try session.begin("repl:1:> ", w80, null);
    _ = session.take();
...
test "feed: a line taller than the terminal is drawn in a window that follows the cursor" {
    ...
    _ = try session.begin("> ", small, null);
    _ = session.take();
...
test "feed: Enter on an unfinished form opens a row under the marker" {
    var session = Session.init(std.testing.allocator);
    defer session.deinit();
    _ = try session.begin("repl:1:> ", w80, &balanced);
    _ = session.take();
...
test "feed: Up moves to the row above, and Enter there submits the whole form" {
    var session = Session.init(std.testing.allocator);
    defer session.deinit();
    _ = try session.begin("> ", w80, &balanced);
...
fn submitted(chunks: []const []const u8, lines: *std.Io.Writer.Allocating) !void {
    var session = Session.init(std.testing.allocator);
    defer session.deinit();
    var ended = try session.begin("> ", w80, &balanced);
    for (chunks) |chunk| {
        ended = try session.feed(chunk, w80);
        while (ended) |_| {
            try lines.writer.print("{s}\n", .{session.line()});
            ended = try session.begin("> ", w80, &balanced);
        }
    }
}
