//! The rune decoder and the character width table the line editor measures
//! with.
//!
//! `layout.zig` and `picture.zig` import this file. A _rune_ is one decoded
//! UTF-8 sequence, or one byte that does not begin a valid sequence. Its
//! _width_ is the number of terminal columns it occupies: 0, 1 or 2.
//!
...
//!   sequence, an overlong encoding, a surrogate and a stray continuation byte
//!   are each invalid.
//!
//! - Every width in the editor comes from `width`, and none from a byte count,
//!   so that the layout and the drawing measure a row the same way.
...
/// The codepoint ranges of width 2, inclusive, sorted and disjoint.
const wide = [_][2]u21{
...
/// The codepoint ranges of width 0, inclusive, sorted and disjoint.
const zero = [_][2]u21{
    .{ 0x0, 0x1f },        .{ 0x7f, 0x9f },       .{ 0x300, 0x36f },     .{ 0x483, 0x489 },     .{ 0x591, 0x5bd },
    .{ 0x5bf, 0x5bf },     .{ 0x5c1, 0x5c2 },     .{ 0x5c4, 0x5c5 },     .{ 0x5c7, 0x5c7 },     .{ 0x610, 0x61a },
    .{ 0x61c, 0x61c },     .{ 0x64b, 0x65f },     .{ 0x670, 0x670 },     .{ 0x6d6, 0x6dc },     .{ 0x6df, 0x6e4 },
    .{ 0x6e7, 0x6e8 },     .{ 0x6ea, 0x6ed },     .{ 0x711, 0x711 },     .{ 0x730, 0x74a },     .{ 0x7a6, 0x7b0 },
    .{ 0x7eb, 0x7f3 },     .{ 0x7fd, 0x7fd },     .{ 0x816, 0x819 },     .{ 0x81b, 0x823 },     .{ 0x825, 0x827 },
    .{ 0x829, 0x82d },     .{ 0x859, 0x85b },     .{ 0x898, 0x89f },     .{ 0x8ca, 0x8e1 },     .{ 0x8e3, 0x902 },
...
/// One decoded rune.
///
/// `decode` returns a `Rune` and `width` takes one. `codepoint` is null when
/// the byte does not begin a valid UTF-8 sequence, and `len` is the number of
/// bytes the rune occupies, which is 1 for an invalid byte.
...
/// Decodes the rune at the start of `bytes`.
///
/// `bytes` must not be empty. This function cannot fail: a byte that does not
/// begin a valid sequence is returned as a rune of length 1 with no codepoint.
pub fn decode(bytes: []const u8) Rune {
    std.debug.assert(bytes.len > 0);
    const invalid: Rune = .{ .codepoint = null, .len = 1 };
...
/// Checks whether `c` is inside one of `ranges`, by binary search.
///
/// `ranges` must be sorted and disjoint, which the test below checks for both
/// tables.
fn within(ranges: []const [2]u21, c: u21) bool {
    var low: usize = 0;
    var high: usize = ranges.len;
    while (low < high) {
        const middle = low + (high - low) / 2;
        if (c < ranges[middle][0]) {
            high = middle;
        } else if (c > ranges[middle][1]) {
            low = middle + 1;
        } else {
            return true;
        }
...
test "decode: valid sequences of each length" {
    try std.testing.expectEqual(Rune{ .codepoint = 'a', .len = 1 }, decode("a"));
    try std.testing.expectEqual(Rune{ .codepoint = 0xe9, .len = 2 }, decode("é"));
    try std.testing.expectEqual(Rune{ .codepoint = 0x5ea6, .len = 3 }, decode("度"));
    try std.testing.expectEqual(Rune{ .codepoint = 0x1f600, .len = 4 }, decode("😀"));
}

test "decode: each invalid form is one byte with no codepoint" {
    const invalid: Rune = .{ .codepoint = null, .len = 1 };
    // A stray continuation byte, a truncated sequence, an overlong encoding of
    // `/`, a surrogate, and a lead byte past U+10FFFF.
    try std.testing.expectEqual(invalid, decode("\x80"));
    try std.testing.expectEqual(invalid, decode("\xe5\xba"));
    try std.testing.expectEqual(invalid, decode("\xc0\xaf"));
    try std.testing.expectEqual(invalid, decode("\xed\xa0\x80"));
    try std.testing.expectEqual(invalid, decode("\xf5\x80\x80\x80"));
    // A lead byte followed by a byte that does not continue it.
    try std.testing.expectEqual(invalid, decode("\xe5a\xa6"));
}
...
test "width: the ends of a range and the codepoints either side" {
    // Hangul Jamo leading consonants are wide and the medial vowels after them
    // combine.
    try std.testing.expectEqual(1, width(.{ .codepoint = 0x10ff, .len = 3 }));
    try std.testing.expectEqual(2, width(.{ .codepoint = 0x1100, .len = 3 }));
    try std.testing.expectEqual(2, width(.{ .codepoint = 0x115f, .len = 3 }));
    try std.testing.expectEqual(0, width(.{ .codepoint = 0x1160, .len = 3 }));
    try std.testing.expectEqual(0, width(.{ .codepoint = 0x11ff, .len = 3 }));
    try std.testing.expectEqual(1, width(.{ .codepoint = 0x1200, .len = 3 }));
    // The last range of each table, and the codepoints past it.
    try std.testing.expectEqual(2, width(.{ .codepoint = 0x323af, .len = 4 }));
    try std.testing.expectEqual(1, width(.{ .codepoint = 0x323b0, .len = 4 }));
    try std.testing.expectEqual(0, width(.{ .codepoint = 0xe01ef, .len = 4 }));
    try std.testing.expectEqual(1, width(.{ .codepoint = 0xe01f0, .len = 4 }));
    try std.testing.expectEqual(1, width(.{ .codepoint = 0x10ffff, .len = 4 }));
}

test "zero, wide: sorted, disjoint, and not overlapping each other" {
    for ([_][]const [2]u21{ &zero, &wide }) |table| {
        for (table, 0..) |range, i| {
            try std.testing.expect(range[0] <= range[1]);
            if (i > 0) try std.testing.expect(table[i - 1][1] < range[0]);
        }
    }
    for (zero) |z| {
        for (wide) |w| try std.testing.expect(z[1] < w[0] or w[1] < z[0]);
    }
}
