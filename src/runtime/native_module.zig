...
fn defs(env: *wattle.Env) wattle.Error!void {
    ...
    wattle.nfuns(env, "zig-native", &.{
        ...
        wattle.reg("keep", &keep, "(keep x [rank])", "An abstract holding x, with every callback set."),
        wattle.reg("kept", &kept, "(kept keeper)", "The value a keeper holds."),
        wattle.reg("rank", &rank, "(rank keeper)", "The rank compare and hash are computed from."),
        wattle.reg("mark-count", &markCount, "(mark-count)", "How many times gcmark has been reached."),
        wattle.reg("finalized-count", &finalizedCount, "(finalized-count)", "How many keepers have been finalized."),
        wattle.reg("unsafe-seen", &unsafeSeen, "(unsafe-seen)", "How many marshal callbacks saw the unsafe flag."),
        wattle.reg("greeting", &greeting, "(greeting)", "A string built by the module."),
        wattle.reg("markup", &markup, "(markup bytes [opts [strict]])", "A byte argument and a tuple of keyword options."),
        wattle.reg("tally", &tally, "(tally dict)", "The sum of a struct's or a table's numeric values."),
        wattle.reg("cut", &cut, "(cut bytes [start [end]])", "A slice of a byte argument."),
        wattle.reg("wrap", &wrap, "(wrap bytes width)", "The first width bytes."),
        wattle.reg("classify", &classify, "(classify x)", "The name of a value's type."),
        wattle.reg("named", &named, "(named x)", "The name of a string, a symbol or a keyword."),
        wattle.reg("peek", &peek, "(peek indexed n)", "The value the keeper at index n holds."),
        wattle.reg("viewed", &viewed, "(viewed x)", "Which Value-form getter reads a value, and its length."),
        wattle.reg("walked", &walked, "(walked indexed)", "An indexed argument read with next, get and nextChunk."),
        ...
        wattle.reg("invoke", &invoke, "(invoke name & args)", "Call the method name on the first of args."),
        wattle.reg("attempt", &attempt, "(attempt f & args)", "Call f on a fresh fiber, returning [signal value fiber]."),
        wattle.reg("status-of", &statusOf, "(status-of x)", "The status of a fiber, refusing anything else."),
        wattle.reg("sort", &sort, "(sort cmp indexed)", "An insertion sort whose comparator is a Wattle function."),
        wattle.reg("kept-across", &keptAcross, "(kept-across f)", "A rooted value carried across a call into f."),
        wattle.reg("unkept-across", &unkeptAcross, "(unkept-across f)", "The same with no root: the case the root exists for."),
        wattle.reg("later", &later, "(later x)", "A value computed on this module's own thread, awaited and woken."),
        wattle.reg("stampede", &stampede, "(stampede n)", "n threads posting at once; returns how many arrived."),
        wattle.reg("abandoned", &abandoned, "(abandoned)", "A wait whose thread posts only once released."),
        wattle.reg("release-abandoned", &releaseAbandoned, "(release-abandoned)", "Let the abandoned wait's thread post."),
        wattle.reg("wake-refused", &wakeRefused, "(wake-refused)", "How many times wake has returned false."),
        wattle.reg("refused-freed", &refusedFreed, "(refused-freed)", "How many contexts the false branch has freed."),
        wattle.reg("loop-available", &loopAvailable, "(loop-available)", "Whether the loop capability is available."),
    });
}
