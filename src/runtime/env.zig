...
//!
//! `coreEnv` has two implementations, chosen by `corefn.bootstrap`: the
...
//! configurations, so this file asks `config.peg` and its kin. `build.zig`
//! decides them and hands them over as comptime booleans, answered in one place.
...
/// written out here; `ev.zig` and `debug.zig` carry them for the same reason.
...
/// than past it, so a template ending in a colon is compared safely and
/// answers no match.
...
/// Collapse `.` and `..` segments in place, walking two indices rather than
...
/// Print and abort rather than raise. Both call
...
/// order is contract only in that `(sandbox ...)` reports an unknown keyword
/// by not finding it, so nothing depends on where a row sits.
...
/// sentinel-terminated array, so the NUL is there and `.len` excludes it;
/// what the runtime hands `unmarshal` is exactly the marshalled stream.
...
/// and diagnostics go to stderr: `vm/entry.zig`'s `continueFiber` answers a
/// `Resumed` and the compiler answers a status, so the failures this function
/// *handles* arrive as values already.
///
