...
/// `api/fingerprint.zig`'s number, as the sixteen digits `wattle/api` is bound
/// to.
...
/// The version and the build identifier, as pointers `strings.cstring` can walk.
///
...
pub fn dobytesImpl(
    ...
    while (!done) {
        while (parser_core.parserHasMore(parser)) {
            ...
            if (cres.status == .ok) {
                ...
                if (resumed.signal != abi.Signal.ok and resumed.signal != abi.Signal.event) {
                    ...
                    errflags |= constants.do_error_runtime;
                    done = true;
                }
            } else {
                ...
                errflags |= constants.do_error_compile;
                done = true;
            }
        }
        ...
        switch (parser_core.parserStatus(parser)) {
            .dead => done = true,
            .@"error" => {
                errflags |= constants.do_error_parse;
...
fn bootstrapCoreEnv(replacements: ?*tables.Table) raise.Error!*tables.Table {
    ...
    registry.def(env, "wattle/api", value.fromBytes(&api_z, .string), "The fingerprint of the native module interface this program was built " ++
        ...
        "program whose wattle/api and wattle/config-bits are the same as the " ++
        "module's own, and that was built with the same version of Zig.");
    registry.def(env, "wattle/config-bits", wrap.fromInteger(constants.current_config_bits), "The set of build options a native module is checked against: " ++
        "whether the build is single-threaded, whether it NaN-boxes, and the " ++
        "pointer shift the NaN box uses.");
...
fn imageCoreEnv(replacements: ?*tables.Table) raise.Error!*tables.Table {
    ...
    // two: `wattle/config-bits`, through the NaN-box pointer shift, and
    // `wattle/api`, whose number is over layouts the target decides.
...
fn loadLibs(env: *tables.Table) raise.Error!void {
    const entries = comptime [_]corefn.Entry{
        ...
        corefn.reg("scan-number", &nfunScanNumber, @src(), "(scan-number str)\n(scan-number str base)", "Parses a number from str, a byte sequence, and returns that number, either an integer " ++
            "or a real. The number " ++
            "must be in the same format as numbers in Wattle source code. Returns nil " ++
            "on an invalid number. If base is given, no " ++
        ...
        corefn.reg("gccollect", &nfunGccollect, @src(), "(gccollect)", "Runs garbage collection. Calling it manually is rarely needed."),
        corefn.reg("gcsetinterval", &nfunGcsetinterval, @src(), "(gcsetinterval interval)", "Sets the number of bytes to allocate before running garbage collection. " ++
            "interval is a non-negative integer. " ++
            "Low values for interval are slower but use less memory. " ++
        ...
        corefn.reg("dyn", &nfunDyn, @src(), "(dyn key)\n(dyn key dflt)", "Gets a dynamic binding. Returns dflt (or nil) if there is no binding for key."),
        corefn.reg("setdyn", &nfunSetdyn, @src(), "(setdyn key val)", "Sets a dynamic binding. Returns val."),
        corefn.reg("trace", &nfunTrace, @src(), "(trace f)", "Enables tracing on f, a function. Returns f."),
        corefn.reg("untrace", &nfunUntrace, @src(), "(untrace f)", "Disables tracing on f, a function. Returns f."),
        ...
        corefn.reg("slice", &nfunSlice, @src(), "(slice ind)\n(slice ind start)\n(slice ind start end)", "Returns the sub-range of ind, an indexed type or a byte sequence, from " ++
            ...
            "Raises if an index is out of range."),
...
fn makeApply(env: *tables.Table) void {
    ...
    quickAsmDef(
        env,
        .{ .tag = constants.fun_apply, .vararg = true },
        "apply",
        1,
        1,
        std.math.maxInt(i32),
