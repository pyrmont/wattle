//! `Buffer`: the growable byte container, its capacity policy, its push
//! primitives and the `buffer/*` surface.
//!
//! `arrays.zig` is the sibling: one data structure with two element types -- a
//! `GCObject` header followed by `count`/`capacity`/`data`, with `data` in a
//! separate heap block reallocated in place. They are separate files because
//! Janet's taxonomy separates them: a buffer is **bytes** and an array is
//! **indexed**. Neither calls into the other; the shared layout is a fact rather
//! than a dependency. A buffer is not traversed here either: `gc/mark.zig` skips
//! its bytes, nothing below marks, and nothing below frees a collectable block.
//!
//! **Nothing here holds anything across a raise.** Three functions raise
//! directly -- `canRealloc`, `pointerUnsafe` and `extra` -- and `gcalloc`
//! can collect, which runs finalizers, which may raise. Every one of those
//! raises happens *before* the allocation it guards: `canRealloc` runs before
//! the reallocation it protects, and `extra`'s overflow check is its first
//! statement. So no path below holds a raw block between acquiring it and
//! storing it where the collector can see it, which is the one shape a skipped
//! cleanup would turn into a leak.
//!
//! **The capacity policy cannot overflow.** `ensure` computes the product in
//! `i64` where both factors fit in 32 bits and clamps at `INT32_MAX`, and the
//! growth is never caller-supplied: there is no `buffer/ensure` binding and
//! every call passes a literal 1 or 2. `array/ensure` *is* reachable from Janet,
//! which is why `arrays.zig` validates its growth and this does not.
//!
//! **`ensure` charges GC pressure before the reallocation** where the array's
//! charges after, so a failed buffer growth is accounted for and a failed array
//! growth is not. Both end the process, so nothing observes it.
...
/// every reallocation of a buffer payload, never after one.
///
/// Three callers below -- `ensure`, `extra` and `cfunBufferTrim` -- and
...
/// payload is not: the foreign flag makes both `deinit` and every growth path
/// leave the foreign pointer alone.
...
/// Release a buffer's payload. Also called from `gc/sweep.zig`'s `deinitBlock`,
/// which is the collectable buffer's only route here.
...
///
/// The product is computed in 64 bits and clamped at `INT32_MAX`, which is the
/// C original's clamp and the width a marshalled buffer carries. A growth of
/// zero or less produced a zero or negative capacity there; **no caller can
/// reach that any more** -- every one in the tree passes 1 or 2, and
/// `DESIGN.md` section 11 ended the C API that was the other way in -- so the
/// conversion that reproduced it is gone with the count's signedness.
...
/// which is why each shifts rather than copying the host's bytes.
...
/// The keyword names the byte order the caller wants, and the answer is whether
/// it differs from the host's -- so `:native` is always false and the other two
...
/// The argument getters have already rejected anything that does
...
fn cfunBufferNew(argv: []repr.Value) align(corefn.alignment) raise.Raising(repr.Value) {
    ...
    // then treats the two alike, which is what C did with the negative.
...
/// so the space is reserved first and the view retaken. Both call sites write
/// it out rather than sharing a helper.
