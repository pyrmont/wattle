...
//! environments that make the two different things -- the last collectable
//! kinds a caller cannot otherwise construct.
//!
//! **A near-miss worth keeping.** `marsh.zig`'s environment lookups look like
//! they belong here and do not: they take a *module environment table*, not a
//! closure environment. Same word, different thing.
//!
//! **Allocation is this file; the frame machinery is `value/fibers.zig`.**
...
//! `value/` after `tables`/`structs`, and Zig is as unbothered by it. A leaf
//! may duplicate a private predicate; it may not duplicate a definition
//! anything else can observe, which is why `stackBytes` is `fibers`' alone --
//! its wrapping on a negative length is behaviour `fibers.setcapacity` depends
//! on, and a second copy that trapped instead would be invisible at both sites.
//!
//! **One call here can raise, and nothing is stranded when it does.**
//! `fibers.reset` reaches `fibers.funcframe`, which packs a variadic tail,
//! which for a struct-argument function hashes the caller's arguments and so
//! may run an abstract type's `hash` callback. Such a callback is out of
//! contract if it raises. The fiber is on `vm.gc.blocks` from the moment
//! `gcalloc` returns, so the collector owns it whether or not this returns, and
//! its `data` array is reachable from the block and freed with it.
...
// environments. The second is a sub-namespace rather than a prefix, so that
// `functions.defs.new()` names the funcdef where a bare `functions.new()`
// would name the wrong type.

pub const defs = struct {
    /// Allocate an empty funcdef. Every field is written, including the ones a
    /// zeroing allocator would have covered, because `gcalloc` does not zero and
    /// the collector traverses a funcdef's pointer fields as soon as it is
    /// reachable.
    ///
...
/// spelling.
///
/// **The store comes before the assertion**, which is an order nothing can
/// observe -- the abort is fatal and the block reaches nothing else -- and
/// which is kept anyway, because "it cannot matter" is not a reason to reorder
/// two writes.
...
///
/// The bytecode and the constants are plain heap allocations rather than
/// collectable ones: a funcdef owns both outright, and the collector frees them
/// with the funcdef.
...
//
// A `FuncEnv` is a *closure's* captured environment, which is why these three
// are here rather than with the fiber machinery they read. The header has the
// dependency that costs and what it buys.
...
pub fn envDetach(maybe_env: ?*FuncEnv) void {
    ...
    // the process ends on the next line -- and the truncation is what a
...
/// the fiber it names still matches it exactly. An environment this rejects is
/// left as the empty off-stack variant.
...
/// A function's environments, as an array the caller indexes. Both shapes the
/// tree wants
...
/// **The fifth flexible array, which is not a head.** A `Function` carries its
/// captured environments in the same allocation, so `envsOf(f)[i]` is the
...
/// not.
///
/// `@offsetOf` and not `@sizeOf`, per `DESIGN.md` section 3: where the two can
/// diverge, the offset is right and the size is wrong. `test/gc_mark.zig`'s
/// `theHeadOffsets` checks the offset the allocator actually used.
...
/// Bit 0 of the GC header's per-type field: the function is traced, and the
/// interpreter prints a line on entry and on return.
...
pub const FuncDef = struct {
    ...
    /// there is no `sourcemap_length` field to read instead.
...
/// `FuncEnv.as`: an environment is either still on a fiber's stack or
