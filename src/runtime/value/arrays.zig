//! `Array`: the growable `Janet` container, its capacity policy, its push/pop
...
//! Janet's taxonomy separates them: an array is **indexed** and a buffer is
//! **bytes**, the distinction the indexed and bytes views draw. Neither calls
...
//! below marks, and nothing below frees a collectable block. **Nothing here
//! holds anything across a raise** either -- `gcalloc` can collect, a finalizer
//! may raise, and `push` raises on overflow, but nothing below holds a raw block
...
//! instance that makes that necessary is here rather than in `buffers.zig`: a
//! growth of zero frees the backing store while leaving `count` untouched -- a
//! use-after-free reachable from pure Janet -- and a negative one asks for
//! almost the whole address space, ending the process where `protect` cannot
//! catch it. `test/buffer_array.zig` pins both refusals, and `ensure` saturates
//! its product rather than trusting it.
//!
//! **Three asymmetries with `buffers.zig` are kept, none observable.** `ensure`
//! charges GC pressure *after* the reallocation where the buffer's charges
//! before, so a failed growth goes unaccounted and both end the process anyway.
//! `initImpl` adds to `vm.gc.next_collection` directly where the buffer's calls
//! `gcpressure`. And `newFrom` charges nothing at all.
...
pub const Array = struct {
    ...
    /// never been grown: `init(a, 0)` leaves `data` null, which
...
/// Build an array from `count` elements. This does not go through `initImpl`
/// and charges no GC pressure for the payload it allocates, which is the third
/// asymmetry the header names.
///
...
/// count four, holding the elements it was handed.
///
/// `strings.new(buf, len)` and `symbols.new(str, len)` have this shape and are
/// still called `new`, because a string constructor copies where an array
...
/// with a negative value -- there is no `array/setcount` binding and nothing in
/// `capi.zig` -- and a caller holding a Janet integer converts at its own
/// checked conversion.
...
/// `array/new` takes a signed integer, and a negative one reserves nothing:
/// `(array/new -5)` is an empty array that every later operation grows from.
/// A capacity is a count here, so the floor at zero is written down rather than
/// arrived at by a negative capacity nothing can satisfy.
...
fn cfunArrayPush(argv: []repr.Value) align(corefn.alignment) raise.Raising(repr.Value) {
    ...
    // 1..-1), so `count + len - 1` is never below `count`. Computing `count - 1`
    // first would underflow on an empty array.
...
/// The aliasing check is not paranoia: concatenating an array onto itself grows
/// it, and the growth may move the payload, so the view is reserved first and
/// then taken again. It is done here rather than in the loop because `push`
