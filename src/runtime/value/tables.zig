...
//! The two call each other -- `toStruct` here calls `structs.begin`,
//! `structs.put` and `structs.end`, and `structs.toTable` calls `put` -- so
//! the two files import each other, which Zig allows and which is why they can
//! be two files at all. Both hold `asSize`, `isNilKey` and `isUnstorableKey`,
//! three inline predicates of two lines each: neither file is the right owner
//! of a rule about the other's keys, and a fifteenth file in `value/` to hold
//! six lines would earn no name.
//!
...
//!
//! **No lookup path here can raise.** `value.dictionaryFind` answers a bucket
//! and `order.equals` answers `bool`; an abstract key's `compare` and `hash`
//! are `callconv(.c) i32` and cannot raise at all, which is the contract
//! `api/abstract_type.zig` states. `rehash` is where that matters.
...
/// requested zero gets *one* bucket, not none, and the `usize` parameter is
/// what makes a negative request unsayable.
///
/// **No constructor here can build a table with a zero capacity**, and that is
/// the reason: `value.mapHash` degenerates into the identity when the mask is
/// all ones, so the whole hash becomes a bucket number and only a hash of zero
/// lands in bounds. A zeroed `Table` still holds those fields, which is how
...
///
/// Tombstones are not carried over, and a rehash is the only thing that ever
...
/// loop runs, because `find` reads it -- and the old array is held only in a
/// local across that loop, which is safe because nothing in the loop can
/// raise. Neither `put` nor `putNoOverwrite` retires a tombstone either: the
/// load factor guarantees an empty bucket and `value.dictionaryFind` prefers
/// one over a tombstone, so the bucket they fill is never a tombstone.
...
///
/// The copy goes through `slots()`, which answers an empty slice when the
/// capacity is zero, so `@memcpy` on a table with a null `data` copies nothing
/// rather than dereferencing the null. `(table/clone @{})` is the caller that
/// reaches it.
...
//
// Each is a `raise.Raising(repr.Value)` and carries its raise out with `try`;
// nothing below holds anything across a call that can raise. The
...
/// `args.optTable` -- that would build an empty table for the default.
