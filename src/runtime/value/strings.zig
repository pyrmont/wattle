...
pub fn lib(env: *tables.Table) void {
    ...
    const trim_doc_tail = "whitespace from bytes, a byte sequence, and returns a string. If chars " ++
        "is provided, trims only bytes that appear in chars.";
    const entries = comptime [_]corefn.Entry{
        corefn.reg("string/slice", &nfunStringSlice, @src(), "(string/slice bytes)\n(string/slice bytes start)\n(string/slice bytes start end)", slice_doc),
        corefn.reg("keyword/slice", &nfunKeywordSlice, @src(), "(keyword/slice bytes)\n(keyword/slice bytes start)\n(keyword/slice bytes start end)", "Behaves as ^string/slice, but returns a keyword."),
        corefn.reg("symbol/slice", &nfunSymbolSlice, @src(), "(symbol/slice bytes)\n(symbol/slice bytes start)\n(symbol/slice bytes start end)", "Behaves as ^string/slice, but returns a symbol."),
        ...
        corefn.reg("string/bytes", &nfunStringBytes, @src(), "(string/bytes bytes)", "Returns a vector of integers that are the byte values of bytes, a string, keyword, symbol or buffer."),
        corefn.reg("string/from-bytes", &nfunStringFrombytes, @src(), "(string/from-bytes & byte-vals)", "Creates a string from integer parameters with byte values. Each integer " ++
            "is reduced modulo 256."),
        corefn.reg("string/ascii-lower", &nfunStringAsciilower, @src(), "(string/ascii-lower bytes)", "Returns a new string where all bytes are replaced with the " ++
            "lowercase version of themselves in ASCII. Does only a very simple " ++
            "case check, meaning no unicode support."),
        corefn.reg("string/ascii-upper", &nfunStringAsciiupper, @src(), "(string/ascii-upper bytes)", "Returns a new string where all bytes are replaced with the " ++
            "uppercase version of themselves in ASCII. Does only a very simple " ++
            "case check, meaning no unicode support."),
        corefn.reg("string/reverse", &nfunStringReverse, @src(), "(string/reverse bytes)", "Returns a string that is the reversed version of bytes."),
        corefn.reg("string/find", &nfunStringFind, @src(), "(string/find patt bytes)\n(string/find patt bytes start)", "Searches for the first instance of pattern patt in " ++
            "bytes, starting at index start, which defaults to 0. Returns the index in bytes of the first byte of the match if found, " ++
            "otherwise returns nil. patt must not be empty."),
        corefn.reg("string/find-all", &nfunStringFindall, @src(), "(string/find-all patt bytes)\n(string/find-all patt bytes start)", "Searches for all instances of pattern patt in " ++
            "bytes, starting at index start, which defaults to 0. Returns an array of all indices of found patterns. Overlapping " ++
            "instances of the pattern are counted individually, meaning a byte in bytes " ++
            "may contribute to multiple found patterns. patt must not be empty."),
        corefn.reg("string/has-prefix?", &nfunStringHasprefix, @src(), "(string/has-prefix? pfx bytes)", "Checks whether bytes starts with pfx."),
        corefn.reg("string/has-suffix?", &nfunStringHassuffix, @src(), "(string/has-suffix? sfx bytes)", "Checks whether bytes ends with sfx."),
        corefn.reg("string/replace", &nfunStringReplace, @src(), "(string/replace patt subst bytes)", "Replaces the first occurrence of patt with subst in bytes. " ++
            "If subst is a function, it is called with patt only if a match is found, " ++
            "and returns the actual replacement text to use. " ++
            "Returns the new string if patt is found, otherwise returns bytes as a string."),
        corefn.reg("string/replace-all", &nfunStringReplaceall, @src(), "(string/replace-all patt subst bytes)", "Replaces all instances of patt with subst in bytes. Overlapping " ++
            ...
            "and returns the actual replacement text to use. " ++
            "Returns the new string if patt is found, otherwise returns bytes as a string."),
        corefn.reg("string/split", &nfunStringSplit, @src(), "(string/split delim bytes)\n(string/split delim bytes start)\n(string/split delim bytes start limit)", "Splits bytes with delimiter delim and returns an array of " ++
            "substrings. The substrings do not contain the delimiter delim. If delim " ++
            "is not found, the returned array has one element. delim must not be empty. Starts searching " ++
            "for delim at the index start (if provided), and returns up to a maximum " ++
            "of limit results (if provided). When limit is reached, the last element holds the rest of bytes."),
