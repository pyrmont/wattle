...
///
/// **The type mask is a `repr.TagSet`**, so `cslot` builds one with
/// `TagSet.one` and "any type" is `TagSet.all`. A raw tag written where a mask
/// is meant has no spelling against this layout.
pub const SlotFlags = packed struct(u32) {
    ...
    /// Everything but the type mask, which is what `janetc_sequal` compares:
    /// `lhs.flags & ~JANET_SLOTTYPE_ANY`.
...
/// A compiler scope. **Not `extern`**: its four vectors are
/// `std.ArrayListUnmanaged`, which holds a slice, and a slice has no C
/// representation, and nothing needs one: every use of this type is a local
/// `var` or a pointer.
///
...
pub const Compiler = struct {
    ...
    /// The index the next instruction will occupy, as the **signed** label
    /// the jump arithmetic uses.
    ///
...
///
/// Neither slot is `callconv(.c)`: the table is built in `compiler/optimize.zig`
/// and read in `compiler.zig`, and nothing outside this tree fills one in.
...
/// What a new binding hides, if anything. `janetc_shadowcheck` answers it and
/// `shadowLint` is the only reader; the numbers are internal to the compiler
/// and nothing serializes them, but they are written out because the C enum
/// they come from fixes them by declaration order.
...
/// A nil, named so the emitter's error paths read as one thing.
...
// otherwise.
//
// **This is not variadic.** A variadic *definition* is out of reach anyway:
// Zig 0.16 cannot name a `va_list` on `aarch64-linux`. Nothing outside the
// compiler front end calls this, so its argument list is an ordinary Zig
// tuple that the compiler counts and
// type-checks.
//
...
///
/// The `lints == null` test comes first because `pp_format.formatc`
...
/// The same, for a lint with nothing to interpolate.
///
...
/// A fresh register, or nothing when the scope has run out of them.
///
/// It answered `undefined` on the failure path and thirteen callers read the
/// result -- poison in a safe build, and a slot index nobody chose in a fast
...
pub fn defAddflags(definition: *functions.FuncDef) void {
    // The seven "has" bits say which optional parts the definition carries, so
...
///
/// Three outcomes, one of them carrying a payload, which is what makes it a
/// union rather than a flag beside two out-parameters.
...
/// Abort unless `condition`. Not a raise: a broken scope chain is a defect in
/// this file rather than a program error. Same shape as `peg.zig`'s
/// `pegAssert` and `marsh.zig`'s `marshAssert`.
...
///
/// Answers nothing having recorded a compile error: a failure here is recorded
/// on the compiler rather than returned, and the absent binding is what the
/// caller acts on. The error union is a different channel: rendering the
...
/// including the lints key that may never have been set.
fn runMacro(
    compiler: *Compiler,
