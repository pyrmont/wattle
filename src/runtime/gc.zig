...
//! allocator.
//!
//! `gcalloc` and `gcallocWithPayload` are what a caller allocating a value
//! reaches for, and both go through `gcallocBytes`, which prepends the block
//! to `vm.gc.blocks` or to `vm.gc.weak_blocks`. `gcroot`, `gcunroot` and
//! `gcunrootall` are the root set. `gclock` and `gcunlock` suspend collection.
//! `smalloc`, `srealloc`, `sfree` and `sfinalizer` are the scratch allocator,
//! with `scratch_heap` as the same allocator behind `std.mem.Allocator`.
//!
//! Traversal and reclamation are `gc/mark.zig`'s and `gc/sweep.zig`'s. Nothing
//! here traverses or frees a collectable object. `gcallocBytes` returns raw
//! memory with only its type tag written: the caller initialises the block,
//! and the collector does not look at it until it is reachable. The block
//! lists are touched at one end only, this file prepending and
//! `gc/sweep.zig`'s `sweep` unlinking.
//!
//! ## Nothing here is stranded by a skipped cleanup
//!
//! One call reaches code this runtime does not own: `freeOneScratch` runs a
//! `ScratchFinalizer` that an embedder installed through `sfinalizer`. Nothing
//! under `src/` installs one, and `test/gc_alloc.zig` is the only caller of
//! `sfinalizer` in the tree. The type is `callconv(.c) void`, so a finalizer
//! has no way to raise, and `freeAllScratch` empties the table after the loop
//! rather than during it.
//!
//! ## The header size is added with a checked add
//!
//! A request within a header of `maxInt(usize)` becomes an out-of-memory
//! rather than a small allocation the caller reads as addressing the whole
//! range. That is the exit `scalloc` already takes for its own multiplication.
...
/// Bytes allocated before the first collection, and the only field of a fresh
/// collector that is not a zero.
...
/// `c_longlong` array and `extern` fixes the order.
...
/// Janet's scratch heap, as the standard interface.
///
/// Stateless, so `ptr` is `undefined` and must never be read. It is not
/// stateless the way `utils.heap` is, though: every call reads
/// `vm_state.current().scratch`, so this allocator is usable only once a VM
/// exists, exactly as `smalloc` is.
///
/// It is a second vtable rather than a parameter on `utils.heap`'s because the
/// two allocators differ in lifetime. A scratch block is released by the next
/// collection whether or not anyone freed it, and a `utils.heap` block is not.
///
/// Which of the two a container uses is a correctness question. The growable
/// vectors this exists for belong to the compiler, the marshaller and the PEG
/// builder, and each of those raises between allocating and freeing:
...
/// scratch heap that is bounded, because the next collection takes it. On
...
/// The vtable behind `scratch_heap`.
///
/// `smalloc` and `srealloc` abort rather than returning null, so `alloc` and
/// `remap` never fail here. The standard signatures still allow failure, and a
...
/// A finalizer an embedder installs on a scratch block through `sfinalizer`.
/// `freeOneScratch` runs it as the block is released.
pub const ScratchFinalizer = ?*const fn (?*anyopaque) callconv(.c) void;

/// The scratch table: the blocks freed together at the next `freeAllScratch`
...
///
/// The lifecycle is `collectorInit` and `gc/sweep.zig`'s `clearMemory` rather
/// than a method here, because a `deinit` needs Janet's allocator and the two
/// calls are already the ones `vm/lifecycle.zig` makes.
pub const Collector = struct {
    ...
    /// Bytes allocated since the last collection, the threshold that ends the
    /// interval, and how many blocks are on the two lists.
    /// Bytes allocated since the last collection, and the threshold that ends
    /// the interval.
    next_collection: usize = 0,
    interval: usize = 0,
    block_count: usize = 0,
    /// Nesting depth rather than a flag: `gclock` returns the previous value
    /// and `gcunlock` restores it, so a scope may nest inside another.
    suspend_count: u32 = 0,
    ...
    ///
    /// `collectorInit` sets it and the field default deliberately does not.
    /// The VM storage is `std.mem.zeroes(Vm)`, which does not apply a field
    /// default, so a guard written as the default would make `Vm{}` disagree
    /// with the zeroed storage a fresh thread gets, which `test/vm_state.zig`
    /// compares, while leaving the storage at zero anyway. `collectorInit`
    /// runs on every VM cycle, and nothing can mark before it: `gcallocBytes`
    /// refuses to run in an uninitialised VM.
    depth: u32 = 0,
...
/// flags.
///
/// It is declared here rather than in `abi.zig` because nothing an author
...
/// the payload would be written over `finalize`.
...
/// that state, `signal.zig` restoring the depth on an unwind, and zeroing the
/// field is still what makes the starting state one statement rather than six
/// assignments and an omission.
pub fn collectorInit(g: *Collector) void {
    g.* = .{ .interval = default_interval, .depth = config.recursion_guard };
}

/// Releases every scratch block named by `table`.
///
/// `gc/mark.zig`'s `collect` calls this at the end of a collection and
/// `gc/sweep.zig`'s `clearMemory` at shutdown. It frees the blocks named by
/// the table it is given, and its caller frees that same table's storage: a
/// call that took its blocks from one table and its pointer array from another
/// would free half of each.
pub fn freeAllScratch(table: *ScratchTable) void {
    for (table.items) |block| freeOneScratch(block);
    table.clearRetainingCapacity();
}

/// Allocates a collectable block that is exactly a `T`, and returns it as one.
///
/// `mtype` is the memory type. Out of memory is fatal inside `rawAlloc`, so a
/// caller writes no `orelse`.
...
/// Allocates a block the collector owns and prepends it to the appropriate
/// heap list.
///
/// `mtype` is the memory type and `size` the whole block in bytes. The block
/// is not initialised beyond its type tag, and it is reachable by the
/// collector from this moment, so every caller fills it in before anything can
/// collect.
///
/// This is the byte-counted form. Three kinds of caller take it: the two typed
...
pub fn gcallocBytes(mtype: MemoryType, size: usize) *abi.GCObject {
    ...
    // embedder who never called `lifecycle.init`. The boundary asks the same
...
/// Allocates a head whose payload sits on the same allocation, and returns the
/// head.
///
...
/// `std.math.add` turns that into the fatal out-of-memory this tree already
/// gives a size it cannot serve.
...
/// Tells the collector that `s` bytes were allocated outside its accounting.
///
...
/// Drops every rooting of `root`, and returns whether there was at least one.
///
/// The index does not advance on a match. `swapRemove` fills the vacated slot
...
/// dangling-pointer-with-a-live-capacity state `scratchDeinit` describes. The
...
/// `smalloc` for `nmemb` elements of `size` bytes, zeroed.
///
/// The multiplication is checked, so a product that would wrap is a fatal
/// out-of-memory rather than a small allocation.
...
/// Releases the scratch table.
///
/// The blocks themselves are `freeAllScratch`'s, which runs first because this
/// frees the table that names them.
///
/// The pointer and the capacity go together. Freeing the storage and leaving
/// the list's pointer dangling with its capacity at the old value lets an
/// `smalloc` before the next VM take the no-growth path and write through the
/// freed pointer.
...
/// Installs a finalizer to run when the block is released, whether by `sfree`
/// or by the next collection.
///
/// `mem` is a pointer `smalloc` returned. It is not tested for null; see
/// `mem2scratch`.
pub fn sfinalizer(mem: ?*anyopaque, finalizer: ScratchFinalizer) void {
    mem2scratch(mem).finalize = finalizer;
}

/// Releases one scratch block before the next collection would.
///
/// `mem` is a pointer `smalloc` returned, or null, which is a no-op. A pointer
/// that is not a live scratch block is fatal rather than diagnosable, for the
/// reason `srealloc` gives.
...
/// optionally before that with `sfree`.
///
/// `size` is the payload in bytes. The header has the finalizer and the table
/// of live blocks has the pointer.
...
/// Resizes a scratch block in place in the table.
///
/// `mem` is a pointer `smalloc` returned, or null, which allocates. A pointer
/// that is not a live scratch block is fatal rather than diagnosable: the
/// table is the only record that a block exists, so a miss means the caller
...
/// Refuses at comptime a `T` that does not put its `GCObject` first.
///
/// `gcallocBytes` writes the memory type through a `*GCObject` at the start of
/// the block and the sweep recovers the header the same way, so the cast back
/// to `*T` is sound only while the header is at offset zero. The heap types
/// are ordinary auto-layout structs, `extern` being kept only where a layout
/// is a stated obligation, so Zig is free to reorder their fields and nothing
/// but this would notice. A reorder is a build error here instead of heap
/// corruption at the first collection.
///
...
/// each has a block before it has a type.
...
/// Runs a scratch block's finalizer, if it has one, and releases the block.
/// The finalizer is embedder code; see the file header.
fn freeOneScratch(s: *ScratchBlock) void {
    if (s.finalize) |finalize| finalize(scratchData(s));
    utils.free(s);
}

/// Identity for root bookkeeping.
///
/// The three immediate types compare equal to any other value of their type,
/// which costs nothing: the collector never traces them, so which value a root
/// slot has cannot matter.
...
/// The inverse of `scratchData`.
///
/// The subtraction wraps, so a null argument produces a wild pointer rather
/// than trapping here: `sfinalizer` does not test for null, and the fault
/// belongs at its store rather than at this line.
...
///
/// It is an offset and not a `@sizeOf` because the compiler reports where it
/// put the payload rather than where it ought to go, and the zero-length field
/// is kept in order to be able to ask.
///
/// Finding the field rather than naming it is what makes every head one case:
/// `functions.Function` spells it `_envs` and the others spell it `_data`, and
/// a head that grew a second flexible member, or lost the one it has, is a
/// compile error here rather than a wrong size.
...
/// `scratch_heap`'s in-place resize entry point.
