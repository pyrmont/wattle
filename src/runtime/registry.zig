...
//! cfunction pointer and holding the name, prefix and source
...
//!
//! **`textSubstitution` runs arbitrary Janet code**: a call for a function, and
//! a cfunction pointer for a builtin. The second goes through
//! `raise.cfunction`, which is what makes forgetting the raise impossible;
//! the note at `textSubstitution` says what forgetting it costs.
...
/// `sortRows` clears it. It is the reason this is a struct holding a
...
/// than eight assignments at VM start-up and two at shutdown.
///
...
///
/// Insertion sort, which is right for the shape of the input: the registry is
/// filled once at startup in whatever order the libraries register, and then
/// appended to rarely.
///
...
///
/// The growth is `(count + 1) * 2` with a floor of 512, which is sized to the
/// core: it registers a little over three hundred builtins, so the floor means
/// one allocation for the whole startup rather than nine.
///
...
/// The ambient entry point: it finds the current VM's registry itself. A caller
/// that already has the table calls `putRow`.
pub fn registryPut(
    key: abi.CFunction,
    name: ?[*:0]const u8,
    name_prefix: ?[*:0]const u8,
...
/// `dirty` flag are maintained for. A linear walk would be O(n) over three
/// hundred rows, per frame, on the path a stack trace takes to name each one.
fn getRow(r: *Registry, key: abi.CFunction) ?*Row {
    ...
    while (lo < hi) {
        ...
        if (rows[mid].cfun == key) {
            ...
            // landed on.** A bisection over a run of equal keys may land
            // anywhere in the run, so walking back to its start is what makes
            // the answer independent of the search path.
            //
            // Which registration that first row holds is not a property this
            // code has. One cfunction may be registered more than once under
            // different names, and `sortRows` shifts past equal keys rather
            // than stopping at them, so every re-sort -- one per `registryPut`
            // that dirties the array, and a module load does one -- reverses
            // the order of a tie. That is what a Janet program sees. The
            // property that holds either way is the one `test/registry.zig`
            // pins: the row a walk of the array would find first.
            var first = mid;
...
/// The ambient entry point: it finds the current VM's registry itself.
...
/// steal its low bits.
///
/// Registration is where the check goes rather than every wrap, because a
/// cfunction pointer and an abstract type pointer are each registered exactly
/// once and wrapped repeatedly afterwards.
...
///
/// **`DESIGN.md` section 6.** One `Reg` and one loop over one axis -- whether
/// each name is prefixed -- because `def(env, n, f, doc)` *is*
/// `defSm(env, n, f, doc, null, 0)`, and a source location the caller does not
/// have is two nulls rather than a second entry point.
///
/// It is a struct rather than a function because a caller with a sentinel-
/// terminated table cannot hand over a slice: `capi.zig`'s `janet_cfuns_ext`
...
// The two forms below are the runtime's; `defSm` and `Installer` above are the
// bootstrap's, and `corefn.zig` picks between them and records why both arms
// are real.
//
...
// build is the core lookup dictionary, which is what the unmarshaller resolves
// the image's symbol references against.
...
/// Record an abstract type under its name, so that the unmarshaller can find
/// it again.
///
...
/// Resolve a symbol, dereferencing the two dynamic forms.
///
/// The same `Binding` `resolveExt` answers, with `value` already read out of
/// the dynamic cell.
...
///
/// The replacement is what "memoize" means here: the caller's `Value` is
...
///
/// **The cfunction call goes through `raise.cfunction`, and Janet's does
...
/// What a resolved binding is. `resolve` is the only thing that produces one
/// -- it reads an environment entry's keys and decides -- so nothing outside
/// this runtime can supply a value and the enum is exhaustive. The width is
/// unchanged, because `Binding` is `extern`.
