...
//! `%K`, `%w`, `%y` and the eight spellings of pretty-printing.
//!
...
pub fn bufferFormat(
    ...
    while (strfrmt[at] != 0) {
        ...
        switch (conversion) {
            ...
            'M', 'm', 'N', 'n', 'Q', 'q', 'P', 'p', 'w', 'y' => try renderPretty(
                b,
                conversion,
                &spec,
                argv[arg],
...
/// Renders the eight pretty conversions and the two data notations, which both drivers reach the
/// same way.
///
...
fn renderPretty(b: *buffers.Buffer, conversion: u8, spec: *const Specifier, x: repr.Value, startlen: usize) raise.Error!void {
    if (conversion == 'w' or conversion == 'y') {
        var depth = Specifier.number(&spec.precision);
        if (depth < 1) depth = recursion_guard;
        _ = if (conversion == 'w')
            try pretty.source(b, depth, x, startlen, b.count)
        else
            try pretty.edn(b, depth, x, startlen, b.count);
        return;
    }
