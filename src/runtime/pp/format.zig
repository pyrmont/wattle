...
//! specifier.
//!
//! Adapted from Lua's `lstrlib.c`. A specifier is rewritten into an ordinary C
//! one and handed to `c.snprintf` for the numeric and string conversions; the
//! thirteen Janet-specific ones -- `%v`, `%V`, `%t`, `%T`, `%j` and the eight
//! spellings of pretty-printing -- are rendered here.
//!
//! ## The arguments are a comptime tuple, not a `va_list`
//!
//! `formatTuple` takes the caller's tuple and the format string is `comptime`,
//! so the walk happens once at compile time and the engine *indexes* its
//! arguments rather than pulling them. **A conversion handed the wrong type or
//! the wrong width is a compile error at the call site**, where a variadic
//! would read whatever the caller happened to push.
//!
//! A variadic is also not available to fall back on: Zig 0.16 cannot name a
...
//! `%D` and `%d` are separate cases in the first and one case in the second;
//! `%s` reads a `const char *` in the first and a Janet value through
//! `args.getCBytes` in the second; `%S` and `%T` exist only in the first, and
//! only the second can run out of arguments. They stay two loops sharing the
...
/// The scratch one rendered conversion goes into.
const max_item = 256;
/// The scratch the rewritten specifier is built in.
const max_format = 32;
/// The flag characters a specifier may carry, and the count that makes a
/// repeat detectable.
const fmt_flags = "-+ #0";
/// The conversions whose length modifier is rewritten.
const fmt_replace_inttypes = "diouxX";

/// The page width and the recursion budget a pretty conversion is given when
/// the specifier names neither.
...
///
/// Only the engine knows what type comes next, because only the engine has
/// parsed the format string, and a `va_list` cursor can only be advanced
/// forwards at runtime. A tuple is indexed instead, so the walk happens once,
/// at compile time, and what comes out is a straight line of literal pushes
/// and renders.
const Op = union(enum) {
...
inline fn renderConversion(
    ...
    switch (conversion) {
        ...
        // caller passed is the value that renders, at the width it was passed.
...
/// Append a formatted message to a buffer.
///
...
/// Render into a scratch buffer and return a Janet string.
///
/// The `errdefer` is what keeps a raise from `%v`'s `tostring` callback from
/// stranding the scratch buffer; nothing a program observes depends on it.
...
/// Append to a buffer the caller owns, and hand it back.
pub fn formatb(
...
/// Write a formatted message to whatever `(dyn name)` names, falling back to
/// `dflt_file` when the dynamic binding is absent.
///
/// `name` is a runtime pointer because null and the empty string both mean
/// "use the default directly"; `format` is `comptime`, and every caller passes
/// a literal.
///
...
/// checked, which is what decides the message a non-writeable file raises.
pub fn dynprintf(
    name: ?[*:0]const u8,
...
///
/// One caller: `bytecode.zig`, whose assembler answers a message pointer
/// rather than an error union, so a raise from the formatter has no channel to
/// travel in. Everywhere else the kernel is reached by `@import` and the error
/// is returned.
///
/// It records the raise and returns a blank string, which nothing consumes:
/// the process then dies at the next protected scope. One instance is what
/// that shape is down to.
pub fn formatcReported(comptime format: [:0]const u8, args: anytype) strings.String {
    return raise.reported(formatc(format, args));
}

/// Raise with a formatted message: the runtime's most-used raise, at nearly
/// two hundred call sites.
///
/// **It is here rather than in `raise.zig`** because it needs the pretty
/// printer -- `%v` and the eight spellings of `%q` run it -- and `raise.zig`
/// is the shared mechanism rather than a subsystem that may depend on one.
...
/// `Janet` array. `first` is the index of the first value the format string
/// consumes.
///
/// **It is the index of the first value, not of the one before it**, and the
/// step happens at the bottom of each conversion rather than the top. That is
/// what lets the index be unsigned: an index of the one before it is -1 for a
/// caller whose `argv` holds no format string at all, which `test/pp_format.zig`
/// passes. Stepping at the bottom says the same thing about the same values,
/// because every path between the bounds check and the step either uses `arg`
/// or returns.
pub fn bufferFormat(
    b: *buffers.Buffer,
    strfrmt: [*]const u8,
