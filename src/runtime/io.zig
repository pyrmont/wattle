...
/// type, and one declaration checked against the other is exactly the mismatch
...
/// no-nil flags, and it collapses append and write, because all `c.fdopen`
/// has to do is accept the result.
...
// The stream is optional throughout the kernels below, because a closed
// `File` holds a null. **No caller here reaches one**: `flusher` skips a
...
/// Write `count` bytes as a single item, so the result is 1 on success and 0
...
/// for this, which is why it is set separately.
...
/// The one path that holds a stream as a bare pointer rather than as a `File`:
...
/// Print where and why, then end the process.
///
...
/// to `fprintf`, because `fprintf` takes the *translated* `FILE *` and this
/// file deliberately does not name that type.
...
///
/// **The interleaving is contract and it matters**: the permissions the
...
/// printed nothing -- which is contract, and is why the `default` arm below
/// cannot simply own every non-file case.
fn printImplX(
    argv: []repr.Value,
    newline: bool,
    dflt_file: ?*FILE,
...
fn printfImplX(
    ...
    switch (repr.typeOf(x)) {
        ...
        repr.Tag.abstract => {
            ...
            // `assertWriteable`, which would say "file is closed". The two
            // messages are contract, so the test stays separate.
    ...
    // past the release below. Nothing leaks -- `buffers.new` gcallocs, so
    // `gc/sweep.zig`'s `buffers.deinit` arm frees the backing store at the
    // next collection -- and the release here is what returns it sooner.
...
// ==========================================================================
// The file surface other subsystems reach
//
// None of these is exported: `capi.zig` is the only file under `src/` that
// exports, and it publishes none of them. `os/process.zig` reaches
// `makejfile` by import, and `test/io_core.zig` is the contract over the rest.
// ==========================================================================
...
/// reaching through this accessor has no flag word unless it asks for one, so
/// without the test a caller that does not ask hands `fileno` a null. The test
/// belongs here, where every such caller inherits it.
