...
//!
//! **The scratch buffer is released on every path.** `description` and
//! `toString` initialise a `buffers.Buffer` on the stack and call the
//! `raise.Raising` half through `raise.reported`, which catches, so the
//! `intern` below it -- and the `buffers.deinit` inside it -- runs whether or
//! not a `tostring` callback raised. What a raise does leave is a report,
//! which the caller of these two must consume.
...
/// Returns the number of columns the escaped form occupies, which is what the
/// pretty printer adds to its alignment.
///
...
/// Render into a scratch buffer and hand back an interned string.
///
/// Both callers reach this after a `raise.reported`, which catches, so the
...
/// Like `description` above, except that the three byte-sequence types and a
