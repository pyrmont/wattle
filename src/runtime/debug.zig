...
// Breakpoints
//
// A breakpoint is bit 7 of an instruction word. `vm.zig`'s `runVm` tests it
// before
...
/// funcdef on the heap.
///
/// **The choice rule is contract**, and is worth stating because it is not
...
/// frame that names nothing produces no name and no location, which renders as
/// a bare `  in` line.
pub fn traceFrame(frame: *vm_state.StackFrame, out: *TraceFrame) raise.Raising(void) {
    ...
    // reads it as a code pointer, which is why the cast is here and not beside
    // the frame accessors in `vm/state.zig`.
...
/// gives the order, with no allocation to release -- which matters beyond
...
fn traceChain(fiber: *fibers.Fiber, state: *TraceState) raise.Raising(void) {
    ...
    while (index > 0) {
        ...
        switch (descriptor.loc_kind) {
            ...
            // The widening is contract: `%d` renders the 64 bits the specifier
            // asks for, so the digits are the same for every line number this
            // runtime can produce.
