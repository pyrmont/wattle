...
//! `std.os.linux.Stat`, and `std.Io.File.Stat` carries nine fields where
//! `os/stat` reports fifteen. What it *does* supply on Linux is
//! `std.os.linux.Statx`, which has every field `os/stat` needs.
//!
//! **One field is reconstructed rather than copied**: `statx` reports the
//! device as a major and a minor rather than as a `dev_t`, so `dev` and `rdev`
//! are recombined by `encodeDev` below, which says how and what checks it.
...
/// rather than by pinning the number: **two files on the same filesystem
/// report the same `dev`, and a file agrees with its directory.** A wrong
/// encoding fails that; a different-but-consistent one does not, and is not
/// something this runtime should be asserting.
...
//
// The *name* of the symbol is per-target. On x86-64 Darwin the plain names
// stayed bound to the pre-widening 32-bit `ino_t` structure and the wide one is
// `$INODE64`; arm64 has no such legacy, so there the plain names *are* the wide
// structure. Hence the suffix on x86-64 only -- the same table `std.c` carries.
// Windows has the underscore spellings.
...
/// Stat a path and copy out the mode word and one double per numeric field.
