...
/// A mode as the nine-byte `rwx` string `os/perm-string` answers.
...
/// The same value, converted to what the host's `chmod` takes.
///
...
/// The same with a default for an absent argument.
...
/// Build the Janet value for one field out of what `host_stat.zig`'s
