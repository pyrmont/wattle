...
///
/// The declaration is `abi.Method`, which `src/module.zig` names as well: a
/// module author builds a method table with the same two fields and hands it
/// to the same two entry points, so the layout is agreed rather than declared
/// twice. This is the runtime's name for the same type.
