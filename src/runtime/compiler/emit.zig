...
///
/// The mapping from an error to its text sits next to the code that answers
/// the error, which is possible because the kernels below are reached by
/// import: an error union crosses that and would not cross a C ABI.
///
...
/// message -- said once here rather than reported twice and deduplicated by
...
/// recorded an error instead. A label of zero is a real instruction index, so
/// it is not a sentinel a caller may test: the caller tests the compiler's own
/// result status.
pub fn emitSlot(
    compiler: *compiler_primitives.Compiler,
...
///
/// The range check reports and then emits anyway: only the first error is
/// kept, and the truncated instruction is never run because the compile has
/// already failed.
pub fn emitSl(
    compiler: *compiler_primitives.Compiler,
    operation: constants.Opcode,
