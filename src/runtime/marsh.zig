...
//! inside it, and it reaches out only through public entry points.
//!
//! ## The two directions share less than the file's shape suggests
//!
//! They share the lead-byte vocabulary, the varint encodings
//! (`pushInt`/`readInt`, `push64`/`read64`) and the *shape* of the reference
//! tables -- a growing vector per kind, indices into it on the wire -- and no
//! state type, traversal or error discipline: the marshaller walks live
//! objects and appends, the unmarshaller walks bytes it must not trust and
//! allocates, two halves with a vocabulary between them.
//!
//! ## What can raise through this file
//!
//! Every raise this file *decides* returns an error. **Two** kinds of call
//! inside the traversal can raise something this file did not decide:
//!
...
//!    supplies.
//!
//! **The allocators are not a third kind.** `tables.put` answers `void`,
...
/// Whether this build has the event loop, read as a `bool` constant so each
/// guard below is a plain `if`. The value is comptime-known either way, so a
/// build without the event loop never analyses the arm.
...
/// A fiber in memory never carries the first two, which is why they are put in
/// here and taken out again below rather than being fields, and all three are
/// cleared as they are read back.
...
/// Running out of memory is fatal here rather than raising, so a null
/// allocation ends the process instead of answering one.
...
///
/// No error channel and none needed: nothing it calls has one either.
/// `tables.put` and `value.fromBytes` answer `void`, and `gc.smalloc` is fatal
/// on a size it cannot serve rather than raising.
...
/// The traversal's three reference tables, and two of them are vectors.
///
/// `seen` is a `tables.Table` keyed by value, because a marshalled reference
/// is found by asking "have I written this object". `seen_envs` and
/// `seen_defs` are flat vectors scanned linearly, because a funcenv and a
/// funcdef are not Janet values and cannot be table keys, and because the
/// counts are small. The unmarshaller needs no table at all -- a reference
/// arrives as an index -- so all three of its `lookup`, `lookup_envs` and
/// `lookup_defs` are vectors.
///
/// **The vectors are `scratch_vector.Vector`, and nothing here frees them on a
/// raising path.** Their memory is reclaimed by `gc.freeAllScratch`, which
/// runs at the end of a collection and again at teardown; that is what
...
fn marshalOne(st: *MarshalState, x: repr.Value, flags: c_int) raise.Raising(void) {
    ...
    switch (vtype) {
        ...
        repr.Tag.@"struct" => {
            ...
            // The capacity is not written to the stream -- the reader rebuilds
            // it from the count -- so the walk over it is a plain index, as in
            // the table case above.
        ...
        // exhaustive, and answers what the two unsafe cases answer.
...
/// A pointer to slot `index` of `func`'s environment array, so a caller can
/// assign into it as well as read it.
...
//
// **These re-enter this file's own recursion.** `marshalJanet`, `unmarshalInt`
// and the eighteen others are called *from* an abstract type's callbacks, in
// the middle of the traversal above. Seventeen of the twenty-one are
// `raise.Raising` and return their raise, so a callback's `try` carries it out
...
/// Refuse a read that has reached or passed the end of the stream.
///
/// The bound is checked as an address rather than as a pointer, because two
/// call sites below compute `data - 1 + len` with a length that may be zero;
/// that arithmetic has to wrap rather than trap, and `usize` is where it can.
...
/// measured and decided to keep signed -- `functions.FuncDef`'s `slotcount`
/// and three arities, `fibers.Fiber`'s `frame`, `stackstart`, `stacktop` and
/// `maxstack`, and `vm_state.StackFrame.prevframe` -- plus
/// `functions.FuncEnv.offset`, which is
...
/// `unmarshal` over a pointer and a length, for a caller that has no slice.
///
/// Written out rather than generated: `raise.panicking` copies its parameter
/// types into a `callconv(.c)` abi, and this signature spreads one slice into a
/// pointer and a length, which a slice parameter cannot express.
pub fn unmarshalAbi(
    bytes: ?[*]const u8,
    len: usize,
    flags: c_int,
