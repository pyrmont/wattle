...
/// Every socket this file produces carries `constants.JANET_STREAM_NODUPS`,
/// which is what lets
/// `ev/stream.zig`'s `streamClose` skip the unregister: nothing has duplicated
/// the descriptor, so closing it removes it from the poll set for free.
///
/// **Raising, and it must be**: `registerStream` refuses a descriptor the
...
/// because `ev_callback.EVCallback` is `raise.Error!void` too. A reporting
/// form here would leave the refusal as a report nobody consumes, with a null
/// stream pointer dereferenced on top of it.
...
/// Make sure a socket does not block, and on the platforms that have it ask
/// for `SO_NOSIGPIPE` so that a write to a closed peer is an `EPIPE` rather
/// than a signal.
///
/// **Every result is discarded.** A socket that refuses to go non-blocking is
/// not reported here and shows up later as a would-block that never arrives.
...
/// The options a listening socket wants, and null unless
/// one of them failed.
...
/// The connect callback's state. Only the `ConnectEx` path uses it; the POSIX path passes
/// a null state and reads everything it needs off the stream.
const NetStateConnect = struct {
    overlapped: Overlapped,
};

/// `ConnectEx` is not exported by any import library and
...
/// Put the calling fiber to sleep until the connect completes.
...
/// The accept callback's state. The two platforms share a name and nothing else: the
...
/// Raising, as `acceptWindows` beside it is. An accept callback answers
/// `raise.Error!void`, so an accept whose stream the backend refuses reports
/// `failed to accept connection` rather than carrying on with a null stream.
fn acceptPosix(fiber: *fibers.Fiber, state: *NetStateAccept, event: ev_loop.AsyncEvent) raise.Raising(void) {
    ...
    if (state.function) |f| {
        ...
        // reject it here -- which would be at the first connection, with no
        // caller left to tell.
        const sub_fiber = fibers.new(f, 64, (&streamv)[0..1]) catch unreachable;
        sub_fiber.supervisor_channel = fiber.supervisor_channel;
        ev_loop.schedule(sub_fiber, wrap.fromNil());
...
/// The Windows half: put an accepting socket and a
...
/// Put the calling fiber to sleep on an incoming connection.
...
fn cfunAcceptLoop(argv: []repr.Value) raise.Raising(repr.Value) {
    ...
    // one argument.** Without the upper bound a handler requiring two
...
/// The socket-option table, without a null terminator: a slice carries its own
/// length.
...
/// The `union` `net/setsockopt` builds its value in. `struct ipv6_mreq`
...
/// **The order is contract**: `janet_getmethod` walks the table linearly and
/// `(next stream)` reports it as written.
...
/// Install the `net/` bindings, in upstream Janet's own registration order.
...
/// Winsock has to be started before any socket call, and
...
/// The abstract type a `net/address` answers. Every callback is null, so the
/// name is the whole definition -- and the payload is what `soGetName` reads: a `sockaddr`
/// allocated at the length the platform reported, which is the
/// header-plus-trailing-bytes shape `DESIGN.md` section 3 describes.
...
/// The address family a keyword names. An unrecognised keyword answers
/// `AF_UNSPEC` rather than raising, and that is what a program sees.
...
pub fn socketType(argv: []repr.Value, n: usize) raise.Raising(c_int) {
    ...
    // below rather than the first test.
    if (stype) |wanted| {
...
/// What an address lookup answers.
///
...
/// leave every release site to remember which it held. `free` below reads the
...
pub const AddrInfo = struct {
    ...
    /// The address length `bind` and `connect` are given. That is
...
/// Resolve the host and port arguments at `offset`. Needs
/// `argv.len >= offset + 2`.
pub fn getAddrInfo(
    argv: []repr.Value,
    offset: usize,
...
/// A socket address becomes a `(host port)` tuple, or a
...
/// The stream type, by import. The socket layer exists only where the event
/// loop does, so no configuration needs a symbol to reach it.
///
...
/// A stream's handle as a socket. `host.Handle` is a `void *` on Windows,
...
// ==========================================================================
// The two preprocessor spellings that have no translation
// ==========================================================================

/// Report where the allocation failed and end the process, naming the caller's
/// `@src()`.
