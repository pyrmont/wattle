...
//!
//! **`Channel` is the only declaration of it**, and `unwrap` below is the one
//! cast from the abstract's payload pointer to it.
...
/// The channel at slot `n`, or `dflt` when the slot is absent or nil.
pub fn optChannel(
