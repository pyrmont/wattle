...
/// `InterlockedIncrement` reports the one after, which is why the addend is
/// added back here.
...
/// `epoll_ctl` or `kevent` -- so this is raise-capable, and every caller in
/// the runtime reaches it by import and `try`s it. A reporting form would turn
...
/// Close a stream that carries `constants.JANET_STREAM_TOCLOSE`, once nothing
/// is listening on it.
    ...
    switch (event) {
        ...
        constants.AsyncEvent.init => {
            ...
            if (state.is_buffer != 0) {
                // If a buffer, convert to a string. **Unresolved:** this
                // copies where it could send from the buffer.
