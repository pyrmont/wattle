...
//!
//! `mutexUnlock` raises on a mutex that is not held, and returns that raise
...
/// Unlocking is the one of the twelve that can fail, and it raises by
/// returning.
///
...
comptime {
    // All twelve are reached by import. `mutexUnlock` is the only one that
    // raises, and it returns its raise like everything else here.
}
