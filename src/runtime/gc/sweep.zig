...
//! of the mark phase's boundary and what makes the split hold: the mark reads
//! the object graph and writes one bit per object, and this file reads that bit
...
//! refers to — a read, not a walk, and the reason a weak reference is dropped
//! in the sweep rather than skipped in the walk.
//!
//! `sweep` and `clearMemory` are the file's surface; `deinitBlock` and
//! `checkLiveref` have no caller outside it.
//!
...
//! raise, and the frames a raise would cross own nothing — so what a raise
//! costs is bounded: `deinitBlock` runs *before* the unlink, so the block stays
//! on its list and every later collection finalizes it again.
//!
...
//! table and weak array alive at teardown — 32KB a cycle for a 4096-element
//! weak array — and then nulls the list head, dropping the last pointer to
//! them. `clearMemory` walks both lists with one body, and `test/gc_sweep.zig`
//! pins that both come back empty.
    ...
    switch (gc_alloc.memoryTypeOf(mem)) {
        ...
        gc_alloc.MemoryType.abstract => {
            ...
            if (head.type.gc) |gc| {
                ...
                // sweep finalizes it again. It returns `void`, so a finalizer
                // that fails has no way to say so and nothing here could act on
                // it; `DESIGN.md` section 5 is why the slot is typed that way.
...
/// what may have died. The removal is `tables.remove` with the search
...
fn dropDeadEntries(table: *tables.Table, memtype: gc_alloc.MemoryType) void {
    ...
    // slots for any capacity a table can hold, and a capacity is never negative.
...
///
/// `deinitBlock` runs before the unlink, and that order is load-bearing for
/// the note at the head of this file: a finalizer that
...
/// did not visit.
///
/// The table is a per-collection visit record rather than a heap: `mark`
...
/// Free the whole heap, at VM shutdown.
///
...
pub fn clearMemory() void {
    ...
    // `gc.scratchDeinit` carries: freeing the storage and leaving the list's
    // pointer dangling with its capacity at the old value lets an `smalloc`
    // before the next VM write through the freed pointer. It is heap corruption
    // that only glibc's allocator hardening detects.
    //
    ...
    // what actually catches the mistake.
