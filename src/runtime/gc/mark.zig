...
//! other side of it.
//!
//! **`collect` belongs here rather than with the sweep.** It is the only writer
//! of the collector's two per-collection fields -- the recursion guard and the
//! root count the collection began with -- and `mark` below is their only other
//! reader, so putting it anywhere else would separate the pair.
//!
...
//! nothing, so what a raise costs is a half-marked heap, no sweep, and
//! `next_collection` where it stood.
//!
...
/// `config.ev`. Three parts of the traversal are the event loop's, and one of
/// them reaches a `Vm` field that exists only in that configuration, so this
/// gates compilation rather than merely behaviour.
...
// The recursion guard and the count of roots the current collection began with
// are `Collector` fields -- `gc.zig`'s `depth` and `orig_rootcount`. They are
// per-collection state owned by the collector that spends them, so the two
// functions below reach them through it rather than beside it.
...
/// the value is *rooted* instead of traversed, and `collect`'s second
...
/// array or a detached environment, and the null test is what stops the walk
/// there; `p.?[0..n]` would trap on exactly that case. And a *negative* count
/// reaches this from a malformed fiber, where the mark walk subtracts
/// `JANET_FRAME_SIZE` from a frame address; the answer is an empty run, where
/// `@intCast` would trap.
...
/// Mark a table and its prototype chain, following the chain iteratively: a
...
/// everything added since — including the roots `mark` created when the
