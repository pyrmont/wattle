...
///
/// `index` outlives the loop, which is why it is declared above it: the value
/// it holds when the loop breaks is what decides the result.
...
/// dictionary moves; both the algorithm and the comparison order are kept as
/// upstream has them, because `order.compare` decides key order for every
/// printed table.
pub fn sortedKeys(
...
///
/// Three cases, one per host. Microsoft's `c.strerror` is
...
/// A BSD, Apple included. `arc4random` arrived on macOS at 10.7, which every
/// version this project supports is past, so the test is by family rather
/// than by version.
...
// ------------------------------------------------------- the typed interface
//
// Out of memory is fatal in this runtime: there is no caller anywhere that
// can carry the failure, and `fatal.outOfMemory` aborts. So the check belongs
// at the hook rather than as an `orelse` repeated at every allocating line.
//
// `rawAlloc` and `rawRealloc` are the byte-counted forms the collector and the
// four flexible-array heads want; `alloc`, `allocMany` and `resizeMany` are
...
/// point**: `@intCast` would trap on exactly the values this exists to carry,
/// so the conversion is written out.
///
