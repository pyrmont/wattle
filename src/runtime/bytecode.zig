...
//! the `FuncDef` it hands back -- and an encoder holding the opcode table
...
const Assembler = struct {
    ...
    fn init(self: *Assembler, parent: ?*Assembler, def: *functions.FuncDef) void {
        ...
        // `tables.init` returns the table it initialised, which none of these
        // four call sites wants.
    ...
    /// Release the four tables. The parents are left alone: each is released
    /// by its own frame.
    ...
    /// The index suffix is appended exactly when `errindex`
    ...
    /// The message is already a Janet string and is taken
...
/// Resolves a closure environment by name, walking the
...
/// it". The caller tells the last from the others by testing `< -1`, so the
/// two negatives cannot be collapsed.
fn addEnv(a: *Assembler, envname: repr.Value) i32 {
    ...
    if (envindex >= @as(usize, @intCast(a.environments_capacity))) {
        // **At least one.** A bare doubling is zero for the first
        // environment, which would be a resize to nothing followed by a write
        // into it.
...
/// A lookup that answers nil for anything that is not a table or a struct,
...
/// Walk `environment + 1` links up the parent chain. The `+ 1` is
/// load-bearing: environment 0 means the immediate parent, not the assembler
/// itself.
...
/// The body of one assembly. Every step either
...
fn assemble(a: *Assembler, source: repr.Value, flags: c_int) AsmError!void {
    ...
    // child's result is checked here rather than carried past.
    {
...
/// The entry point. It reports a result rather than raising, which is why
/// nothing underneath it needs an abi and why a caller sees no error union.
...
/// assembler, which is the assembler's own channel. `error.JanetSignal` cannot
