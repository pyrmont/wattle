...
//!
//! All three conventions are compiled and asserted on every target. Everything
//! here is arithmetic over sizes, alignments and ordinals, and nothing about
//! it is architecture-specific except the rules it encodes, while
//! `ffi/call.zig` keeps the conditions that decide which one a build may call.
//! The one genuine host divergence, Apple's departure from AAPCS64 stack
//! packing, is a parameter rather than a conditional, so both variants are
//! reachable from any machine.
//!
//! `Type` and `Struct` do not cross into this file, for the reason
//! `ffi/types.zig` gives: each has a pointer into a garbage-collected
//! abstract. `ffi/call.zig` serialises the type tree into the flat pre-order
//! array of `TypeNode` below, which is scalars only. Classification is
//! genuinely structural, since SysV consults each nested struct's own size and
//! alignment rather than only its leaves, so a flattened list of leaves would
//! not do: the tree has to arrive, just without the pointers.
//!
//! Nothing here raises. A convention that cannot place an argument reports the
//! position and the reason, and `ffi/call.zig` turns that into a panic.
//!
...
//!
//! The SysV rules are the AMD64 ABI Draft 0.99.7, section 3.2.3, Parameter
//! Passing; the AAPCS64 rules are the Procedure Call Standard for the Arm
//! 64-bit Architecture, 2023Q3, section 6.8.2, Parameter passing rules.
...
pub const AllocResult = struct {
    ...
    /// declares the outgoing words as parameters and so needs the count, and
    /// must not include the payloads, which is what would make a large
    /// by-reference argument reach a ceiling not meant for it.
    arg_stack_count: u32,
};

/// One argument, as the conventions see it. `spec` arrives as the class the
/// matching classifier produced and leaves as the placement, which is not
...
pub const ArgSlot = struct {
    ...
    /// aggregate occupies: one per member, which is what section 6.8.2 says
    /// and is not the same as one per eight bytes.
    ///
...
/// begins. `field_count` is zero for everything that is not a struct, which
/// lets a walk skip any subtree without reading what is in it.
///
/// `size` is `types.typeSize`, with the array count already multiplied in,
/// while `struct_size` is the underlying `types.Struct`'s own size with the
/// array count left out. The two conventions each take a different one, and
/// conflating them is the sort of mistake this split exists to prevent.
...
/// alignment. `apple` is that difference as a parameter, so both are
/// reachable from any build. `max_ret_size` is the width of the trampoline's
/// return buffer, which is a host fact and so arrives from the caller.
pub fn allocAapcs64(
    result: *AllocResult,
    ret: *ArgSlot,
    args: []ArgSlot,
    ...
    // The by-reference area follows the stack arguments, so its offsets stay
    // relative until the stack area's final size is settled.
    for (args) |*arg| {
...
/// four or eight bytes wide passed by reference, and a `variant` whose bits
/// say which of the first four arguments are floating point, so that the
/// trampoline loads the vector registers as well.
pub fn allocWin64(
    result: *AllocResult,
    ret: *ArgSlot,
    args: []ArgSlot,
...
/// Round `value` up to the next multiple of `alignment`, which is always a
/// power of two here.
...
/// The index just past `idx`'s subtree. A non-struct node has no fields, so
/// that is the single step which ends the recursion.
    ...
    if (node.struct_size > 8) {
        ...
        while (remaining > 0) : (remaining -= 1) {
            ...
            switch (walk.class) {
                ...
                // aggregate to memory, which is the AMD64 ABI's post-merger
                // rule and what the eight-byte-or-under merge
