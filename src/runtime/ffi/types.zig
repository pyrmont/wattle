...
/// The primitive types a signature can name. The ordinals are contract
...
/// Which of the three conventions this target may call, which is a host fact.
///
...
pub const Mapping = extern struct {
    ...
    pub fn empty() Mapping {
        return .{
            ...
            .offset = 0,
            // Every caller overwrites the whole mapping before reading it, so
            // the zero here is only what an unread field holds.
            .offset2 = 0,
        };
    }
};
...
/// Built from the host's `sizeof` and `alignof`, which
...
// ==========================================================================
//
// Reached by import rather than by symbol, so `Layout` is declared once and
// every call across the group is an ordinary Zig call.
...
/// Build a struct type from the argument list `ffi/struct` was given.
///
...
/// `utils.cstrcmp` compares, so equality here means what equality means
/// there.
///
...
///
/// **All four names decode on every target.** Whether a convention is
/// *enabled* is a separate question and a property of the target; folding the
/// two together is what would leave the conventions a build does not enable
...
///
/// The multiplication wraps explicitly. A count comes from `args.getNat` and a
/// base size from a completed layout, so Janet's own limits keep the product
/// well inside the range; the `*%` is what says a wrap is the answer rather
/// than a trap, which is the behaviour a program already depends on.
