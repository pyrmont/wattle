...
//!
//! ## The stack arguments are placed by the ABI, not by an `alloca`
//!
//! Zig has no `alloca`, so the stack words are declared as ordinary trailing
//! parameters and the compiler puts them where they go: `@Fn` builds the type
//! at comptime and `@call` fills it. Three consequences shape the file, and
//! each is stated where it lives -- **a rung ladder** at `VariantType` and the
//! three ladders below it, because `@Fn` needs a comptime parameter count and
//! a signature's does not exist until runtime; **a frame that outlives the
//! call** at `Frame`, because the by-reference specs store a pointer to a
//! payload the caller wrote; and **a ceiling** at `checkStackCeiling`, because
//! a rung costs superlinearly to compile.
//!
//! ## A narrow argument is extended, not just placed
//!
//! **Extension is the caller's job under every convention here**: a callee
//! that declares `int8_t` is entitled to read the whole register without
//! masking. Storing an `:s8` at its own width sets one byte and leaves the
//! other seven as they were, so `-1` arrives as 255 where the bank is zeroed
//! and as stack residue where it is not. `marshal.writeRegister` is the write
//! a register slot gets; `marshal.writeOne` keeps the type's own width, which
//! is what a struct field and an array element need. Every bank and the frame
//! are zeroed as well, because reading uninitialized memory is undefined.
//!
//! **Marshalling raises, and so does the argument layer.** The frame is
//! released by a `defer` on every such path, so a raise returns it at once
//! rather than leaving it for the next collection; it is scratch either way.
...
// ==========================================================================
//
// `TypeNode`, `ArgSlot` and `AllocResult` are `classify.zig`'s, imported
// rather than restated, so the classifier and the conventions share one
// declaration of each layout and the calls between them are ordinary Zig
// calls.
...
/// slot `i`, which is the order the Win64 register allocator below builds it
/// in.
...
/// The ladders: the outgoing word count is rounded up to a rung and every rung
/// is instantiated, because `@Fn` needs a comptime parameter count. Passing
/// more words than the callee reads is the caller's to push and the caller's
/// to clean up, so the rounding is invisible.
///
/// Each is sized to what its convention can generate. SysV64 passes large
/// aggregates by value on the stack and is the only one that can reach its
/// top rung -- 1024 words, or 8KB of outgoing arguments -- while Win64 and
/// AAPCS64 pass anything large by reference and so are bounded by
/// `types.max_args` at 32 arguments, which is about 128 words.
...
/// of the signature. `gc.smalloc` is what it comes from, so a raise that
/// returns past the `defer` still has it reclaimed at the next collection.
///
/// **The frame outlives the call, and that is why it is not the outgoing area
/// itself.** The by-reference specs store a *pointer* to a payload the caller
/// wrote, and the outgoing area has no address until the call is under way.
/// So the frame is ordinary scratch at the offsets the allocators already
/// compute, the references point into it, and only its outgoing prefix becomes
/// arguments -- the callee gets a copy of that prefix holding pointers into
/// the live original.
...
const Frame = struct {
    ...
    /// Two cases, because there is no `alloca`: a small frame on the caller's
    /// own stack, and a scratch allocation for anything larger.
    ///
    /// A call whose arguments all fit in registers -- which is most calls --
    /// needs no frame at all, and taking one anyway costs a `gc.smalloc`, a
    /// push onto the scratch table, and later a *linear scan* of that table to
    /// find the block again: measured at forty percent of the cheapest call
...
/// How many `TypeNode`s `ty` flattens into: one for a scalar, and one per
/// field at every depth for a struct.
...
/// Flatten `ty` and everything under it into `nodes`, in pre-order.
...
/// How many nodes a type is flattened into before the classifier needs
/// scratch. A scalar is one node and a struct is one per field at every depth,
/// so this covers a struct of thirty-one fields or a nest of the same total
/// width.
const inline_nodes = 32;

/// Ask the convention's classifier what `ty` is.
///
/// The type is flattened into `nodes` first, because a classifier is a pure
/// decision over a flat description and must not see a pointer into a
/// collected abstract.
...
/// ignore because the size already has the count multiplied in, so the byte
/// arithmetic is the right answer for those two. The classifier has already
/// decided this argument is an HFA, so the only question left is how many
/// members it has.
...
/// The classified argument, as the slot the allocators place.
...
/// Turn a convention allocator's failure report into the raise a program sees.
...
/// The ceiling check: past the top rung there is no function type to call
/// through. It runs at description time, so a signature that cannot be placed
/// is refused once rather than on every call.
...
/// Copy the allocator's placements into the signature's mappings. Only the
/// placements travel; the types stay where they were described.
fn applySlots(
    sig_ret: *Mapping,
    mappings: [*]Mapping,
    arg_count: u32,
...
/// branches is spent long before the last of them. An AAPCS64 build never
/// reaches the limit, because that convention has three variants.
inline fn storeReturn(buf: [*]u8, val: anytype) void {
    @setEvalBranchQuota(200_000);
    @memcpy(buf[0..@sizeOf(@TypeOf(val))], std.mem.asBytes(&val));
}

/// The return buffer. Every convention's variants write at offset zero, and it
/// is sized for the widest of them, so one buffer serves all three.
...
fn callSysv64(sig: *Signature, function_pointer: *const anyopaque, argv: []const repr.Value) raise.Raising(repr.Value) {
    ...
    switch (sig.variant) {
        ...
        // A variant no rung matches makes no call at all, and `ret_buf` is
        // zeroed, so the read below answers a zero of the return type.
        else => {},
    }
...
fn callWin64(sig: *Signature, function_pointer: *const anyopaque, argv: []const repr.Value) raise.Raising(repr.Value) {
    ...
    for (sig.args[0..sig.arg_count], 2..) |arg, n| {
        switch (arg.spec) {
            ...
            // Both offsets are the allocator's own, in words, and are used as
            // given.
            .win64_stack_ref => {
...
/// The callback body every trampoline enters.
///
...
/// The JIT-compiled function's abstract type.
...
/// The code address behind a raw pointer or an `ffi/jitfn`.
