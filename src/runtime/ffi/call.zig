...
//!
//! The stack arguments are placed by the ABI rather than by an `alloca`. Zig
//! has none, so the stack words are declared as ordinary trailing parameters
//! and the compiler puts them where they go: `VariantType` builds the type at
//! comptime and `@call` fills it. Three consequences shape the file, and each
//! is stated where it lives. A rung ladder, at `VariantType` and the three
//! ladders below it, because a comptime parameter count is needed and a
//! signature's does not exist until run time. A frame that outlives the call,
//! at `Frame`, because the by-reference specs store a pointer to a payload the
//! caller wrote. And a ceiling, at `checkStackCeiling`, because a rung costs
//! superlinearly to compile.
//!
//! A narrow argument is extended rather than merely placed. Extension is the
//! caller's job under every convention here: a callee that declares `int8_t`
//! is entitled to read the whole register without masking. Storing an `:s8` at
//! its own width sets one byte and leaves the other seven as they were, so
//! `-1` arrives as 255 where the bank is zeroed and as stack residue where it
//! is not. `marshal.writeRegister` is the write a register slot gets;
//! `marshal.writeOne` keeps the type's own width, which is what a struct field
//! and an array element need. Every bank and the frame are zeroed as well,
//! because reading uninitialised memory is undefined.
//!
//! Marshalling raises, and so does the argument layer. The frame is released
//! by a `defer` on every such path, so a raise returns it at once rather than
//! leaving it for the next collection; it is scratch either way.
//!
//! `TypeNode`, `ArgSlot` and `AllocResult` are `ffi/classify.zig`'s, imported
//! rather than restated, so that the classifier and the conventions share one
...
/// top rung, which is 1024 words or 8KB of outgoing arguments, while Win64 and
    ...
    for (sig.args[0..sig.arg_count], 2..) |arg, n| {
        ...
        const to: [*]u8 = switch (arg.spec) {
            ...
            .aapcs64_stack_ref => blk: {
                ...
                // the `aapcs64_stack` case three lines up, and not a word
                // index, which would place the pointer eight times further
                // out.
...
fn callSysv64(sig: *Signature, function_pointer: *const anyopaque, argv: []const repr.Value) raise.Raising(repr.Value) {
    ...
    switch (sig.variant) {
        ...
        // zeroed, so the read below gives a zero of the return type.
        else => {},
    }
...
///
/// AAPCS64 gives an HFA one register per member, which is what this counts.
...
/// arithmetic is right for those two. The classifier has already
...
fn jitfnGetBytes(fun: *const JittedFn, _: usize) []const u8 {
