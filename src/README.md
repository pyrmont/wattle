...
- **There is no C implementation to select, and no Janet C left to call.**
  `src/` is 88 `.zig` files and four hand-written headers: `host/janet_features.h`
  and the host translations `runtime/os/abi.h`, `runtime/net/abi.h` and
  `runtime/filewatch/abi.h`. Any
...
**A directory says which compilation includes its files.** That is the one
structural fact a directory can carry here, and it is the question a module
author has to be able to answer: an author's `.so` compiles `api/` and the two
roots, and must never reach the eighty files beside them.

**It says it; it does not enforce it.** Both package roots sit at `src/`, so a
relative import can cross between the directories and several do by design —
`raise.zig` names four runtime files for the arm a module does not take. What
actually holds the boundary is `tools/check/exports.janet`, which builds the
author package the way an outside author does, and `examples/standalone`, which
consumes this package by path. Making the split compiler-enforced would mean
`api` as a build module with the runtime imported back into it by name; that is
a real option and it is not what is here.
...
`src/root.zig` is the runtime's module root above the three: it names every
file this configuration compiles and it reaches all three directories, so it
...
root file's directory, which is what puts both at `src/` rather than inside a
tier.

`src/api/` is what a module author reads — `abstract_type.zig`, `raise.zig`,
`crossings.zig`, `abi.zig`, `repr.zig` and `constants.zig` — with
`src/module.zig` above them as the package root, which is the only name an
author writes: a module imports `janet` and nothing else. `src/host/` is
`host.zig`, `cabi.zig`, `cabi_check.zig` and the `janet_features.h` the three
translations open with.
...
Nine files outside the subsystems are worth naming:
...
**A subsystem.** Write `src/runtime/<name>.zig`, or a file under the directory its
